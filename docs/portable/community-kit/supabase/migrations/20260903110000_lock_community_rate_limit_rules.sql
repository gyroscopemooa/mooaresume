begin;
revoke all on function public.take_community_rate_limit(text, integer, integer) from authenticated;
drop function public.take_community_rate_limit(text, integer, integer);
create function public.take_community_rate_limit(p_action text) returns boolean language plpgsql security definer set search_path = '' as $$
...
begin
  ...
  case p_action
    when 'POST_CREATE' then v_limit := 5; v_window_seconds := 3600;
