...
export function CommunityLounge({ attachmentNotice = false }: { attachmentNotice?: boolean } = {}) {
  ...
  async function deleteComment(postId: string, commentId: string) {
    if (postId.startsWith("preview-") || !window.confirm("이 댓글을 삭제할까요? 되돌릴 수 없습니다.")) return;
    ...
    setComments((current) => ({ ...current, [postId]: (current[postId] ?? []).filter((item) => item.id !== commentId) }));
    // 글의 댓글 수도 같이 줄여 둡니다 — 서버 트리거가 세는 값과 맞추려고
    // 목록 전체를 다시 불러오는 것은 이 한 줄에 비해 과합니다.
...
function PostCard({ post, comments, commentsOpen, onRecommend, onToggleComments, onSubmitComment, onReport, onDelete, canDeleteComment, onDeleteComment }: { post: CommunityPost; comments: CommunityComment[]; commentsOpen: boolean; onRecommend: () => void; onToggleComments: () => void; onSubmitComment: (postId: string, body: string) => Promise<boolean>; onReport: () => void; onDelete?: () => void; canDeleteComment: (comment: CommunityComment) => boolean; onDeleteComment: (commentId: string) => void }) {
  ...
  return <article className={styles.post}><header><span className={styles.category}>{communityTopicMeta[post.topic].label}</span><span>{post.anonymousAlias}</span>{post.isEditorial && <BadgeCheck className={styles.editorialBadge}/>}<i>·</i><time>{displayTime(post.createdAt)}</time>{onDelete && <button type="button" onClick={onDelete} aria-label="게시글 삭제"><Trash2/></button>}<button type="button" onClick={onReport} aria-label="게시글 신고"><ShieldAlert/></button></header><h2><Link href={communityPostPath(post.id)}>{post.title}</Link></h2><p>{post.body}</p>{post.attachments.length > 0 && <div className={styles.attachments}>{post.attachments.map((file) => <a key={file.id} href={`/api/community/attachments/${file.id}`} target="_blank" rel="noreferrer"><FileText/>{file.filename}<small>로그인 후 열기</small></a>)}</div>}<Link href={communityPostPath(post.id)} className={styles.readMore}>전체 글 보기 <ArrowRight/></Link><footer><button type="button" onClick={onRecommend}><ThumbsUp/> 추천 <b>{post.recommendationCount}</b></button><button type="button" onClick={onToggleComments}><MessageCircle/> 댓글 <b>{post.commentCount}</b></button></footer>{commentsOpen && <div className={styles.comments}>{comments.map((item) => <div key={item.id}><b>{item.anonymousAlias}</b>{item.isEditorial && <BadgeCheck className={styles.editorialBadge}/>}<time>{displayTime(item.createdAt)}</time>{canDeleteComment(item) && <button type="button" className={styles.commentDelete} onClick={() => onDeleteComment(item.id)} aria-label="댓글 삭제"><Trash2/></button>}<p>{item.body}</p></div>)}<div className={styles.commentForm}><input value={comment} onChange={(event) => setComment(event.target.value)} maxLength={1000} placeholder="익명으로 댓글을 남겨보세요."/><button type="button" disabled={sending} onClick={() => void send()}>{sending ? <LoaderCircle/> : <Send/>}<span>등록</span></button></div></div>}</article>;
}
