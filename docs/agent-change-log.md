...
- 변경(Android, `C:.mooaresume-android`, 별도 저장소): 신규 `IntroActivity`가 런처 진입점이 되어 앱을 새로 켤 때마다 `res/raw/mooa_intro.mp4`(웹과 같은 60초 영상)를 재생합니다(사용자 결정: 설치 후 한 번은 너무 적다). 하단 건너뛰기·소리 켜기(기본 무음), 종료·건너뛰기·뒤로가기·재생 오류 시 TWA(`LauncherActivity`)를 새 작업으로 시작합니다. "봤음" 기록 없음. 최근 앱 목록에서 켜진 앱을 다시 여는 경우에는 영상 없음. versionCode 9 / 1.0.8.
- Validation: 에뮬레이터(API 35)에서 서명된 릴리스 APK로 앱·Chrome 완전 종료 후 재실행할 때마다 영상 → 건너뛰기 → 웹 앱 진입 확인. 웹은 tsc·해당 테스트 통과(실행 결과는 커밋 시 확인).
...
- 병합: `git merge --no-ff`(작성자·이력 보존, 충돌 없음, 사전에 `git merge-tree`로 확인). 병합된 커밋: `231b1f7`, `6127aae`, `2adb279`, `28571b7`.
- 실제 효과(순 변경 — CSS 두 줄 + 테스트): PC(701px 이상) `.finalDocument` max-width 820→1000px·좌우 padding 40→8px, `.finalBody p` 문단 간격 18→11px·글자 15→15.5px·줄간격 1.85→1.7·정렬 justify→left(letter-spacing·text-justify 제거). 모바일(700px 이하) 문단 간격 16→11px·줄간격 1.8→1.72(기존 모바일 좌우 padding 20px 유지). 오늘 앞서 배포한 모바일 규칙(번호를 제목 위로 올림, 왼쪽 정렬)과 함께 적용됨. 테스트는 `result-document.test.ts`에 정답 기준 9문단 "지원동기" 회귀 테스트 45줄 추가.
- 상호작용 확인: 오늘 배포한 문단 정리(화면 폭 줄끊김 이어 붙이기)·선택 제안 카드와 같은 파일을 건드리지만 충돌 없음. 정답 기준 9문단은 새 문단 정리 규칙에서도 그대로 유지됨(테스트로 확인).
- Validation: 병합본 `tsc --noEmit` 오류 없음, vitest 164개 파일 중 163개·1,314개 테스트 통과(실패 1개는 이 PC에 Expo 의존성이 없어 나는 무관한 `mobile.test.ts`). 로컬 dev에서 병합 CSS를 잠깐 얹어 375px 모바일 화면 확인(왼쪽 정렬, 글 폭 276px, 줄간격 1.72, 가로 스크롤 없음)과 PC 값 확인(max-width 1000px, padding 6px 8px, left, 15.5px, 줄간격 1.7). PC 큰 화면은 스크린샷이 불안정해 값으로만 확인했고 실제 폰은 확인하지 않음.
- Release: 병합 커밋 + 이 기록 커밋을 `main`으로 fast-forward push → Cloudflare Git 빌드로 배포.
- Rollback: 병합 커밋을 `git revert -m 1`로 되돌리면 글자 모양이 이전(820px, 양쪽 정렬)으로 돌아감.

## 2026-09-25 — Claude: 최종 첨삭본 "선택 제안" 추가 — 접속어로 시작하는 한 줄 문단 붙이기 안내 (추가 전용)

- 배경: 사용자가 "또한 저는 사무직…" 한 줄이 따로 떨어진 것보다 다음 문단에 붙이고 "또한"을 빼는 편이 더 깔끔하다는 의견에 동의하면서도, 첨삭 AI가 취향까지 고쳐 챗GPT식 재작성이 되면 차별점이 사라지므로 **기존 첨삭 결과는 그대로 두고**, 그 위에 컨설턴트식 설명을 **추가**하는 방향을 정함("지금 기능 + 붙인 버전과 이유, 굳이 안 해도 지금처럼도 된다는 취향 존중 문구"). 두 방식 중 A(화면이 정해진 모양을 직접 찾음, AI 호출 없음)로 진행 지시. B(AI가 풍부하게 설명)는 결과가 매번 달라지고 프롬프트·검토 규칙을 건드려야 해서 보류.
- 변경(추가 전용): `src/domain/result-document.ts`에 `suggestConnectorMerges`(+ 타입 `ConnectorMergeSuggestion`) 추가. 신규 `src/components/connector-merge-hint.tsx`·`connector-merge-hint.module.css`(카드: "작성 팁 · 선택 사항", 안내 문구, "붙인 모습 보기"(미리보기), "이렇게 바꾸기", 적용 뒤 "되돌리기"). `src/components/result-workspace-complete.tsx`는 import 1줄, 상태·핸들러(`mergeUndo`, `applyConnectorMerge`, `undoConnectorMerge`), 최종 첨삭본 본문 아래 JSX 1개만 추가 — 기존 줄 삭제·변경 없음.
- 동작: 최종 첨삭본 탭의 각 문항에서, ① 접속어(또한·그리고·아울러·더불어·게다가·이와 함께·한편)로 시작 ② 한 문장 ③ 100자 이하 ④ 첫·마지막 문단이 아님 ⑤ 바로 뒤 문단이 더 김 ⑥ 뒤 문단이 "첫째/둘째…" 나열이 아님 — 을 모두 만족하는 문단이 있으면 접속어를 빼고 다음 문단 맨 앞에 붙이는 안을 제안. "제가 지원하는 이유는 두 가지입니다."처럼 접속어 없는 도입 문장은 대상 아님. 같은 글이면 항상 같은 결과(정해진 규칙). 카드 문구에 "지금 그대로도 괜찮으니 선택은 자유예요" 포함. "이렇게 바꾸기"는 기존 `answers` 상태(지원자가 문항별 첨삭에서 고치는 것과 같은 경로, sessionStorage 저장)만 바꿔 화면·복사·TXT·DOCX에 반영. 되돌리기는 적용 직후 글과 같을 때만 가능해 그 뒤에 직접 고친 내용을 덮어쓰지 않음. 관리자 미리보기(`adminPreview`)에는 표시 안 함, 인쇄 시 숨김.
- 변경하지 않은 것: AI 프롬프트·검토 AI(`revision-quality.ts`)·스키마·저장된 결과 데이터·첨삭 결과 문장 자체. 제안은 결과를 바꾸지 않고 지원자가 누를 때만 적용됨.
- 알려진 한계: 정해진 모양만 찾음(그 밖의 문체 제안은 없음). 설명 문구는 정형이며 AI 컨설턴트식 풍부한 설명이 아님. 한 번에 제안 하나만 표시. "다음 문단에 붙이기"만 제안하고 앞 문단에 붙여야 할 "또한"인지는 판단하지 못함 → 미리보기와 "그대로도 괜찮다"는 문구로 지원자가 판단. 예정(미구현): 화면이 찾은 자리에 한해 AI가 컨설턴트식 설명 한 문단을 쓰는 하이브리드 — 찾는 위치는 항상 같고 문구만 풍부해지며, 결과에 한 번 저장해 다시 열 때 바뀌지 않게 하는 안.
- Validation: `origin/main`(`b12ab58`) 기준 격리 worktree에서 `tsc --noEmit`·ESLint 오류 없음, 전체 vitest 164개 파일 중 163개·1,310개 테스트 통과(실패 1개 파일은 이 PC에 Expo 의존성이 없어 나는 `mobile.test.ts`로 변경 무관). 신규 테스트: `result-document.test.ts` 8개(사용자 자소서 그대로 재현, 원문 불변·글자 보존, 결정성·재제안 없음, 쉼표·다른 접속어, 도입 문장·첫/마지막·여러 문장·긴 문단·문단 안 "또한"·짧은 뒤 문단·둘째 나열 제외), `result-workspace-complete.test.tsx` 4개(제안 표시, 적용·되돌리기, 제안 없음, 관리자 미리보기). 로컬 dev에서 375px 모바일로 실제 화면 확인 — 카드 모양, 미리보기 펼침, "이렇게 바꾸기"로 문단 4→3·저장 값 반영, "되돌리기"로 4문단 복원, 가로 스크롤 없음. 콘솔 오류는 LIVE-SUB 런타임의 localhost CORS뿐(이 변경과 무관). 실제 폰과 PC 큰 화면에서는 눈으로 확인하지 못함.
...
- 원인: 원문(PDF·한글 등에서 복사한 글)이 화면 폭마다 줄이 끊긴 채 들어왔고(스크린샷 줄들의 표시 폭이 144~148칸으로 비슷함 → 고정 폭 줄끊김), AI 첨삭본(`revisedAnswer`)이 그 줄바꿈을 그대로 따라옴(서버·프롬프트에는 줄을 끊는 코드 없음 확인). 2026-09-24 항목의 `normalizeAnswerParagraphs`가 "줄바꿈 1개 이상 = 문단 경계"로 처리해서 이 줄끊김이 전부 별도 `<p>`로 갈라짐(그 변경의 부작용).
- 변경(표시 계층만, 저장 데이터·AI 프롬프트·검증기·CSS 변경 없음): `src/domain/result-document.ts`의 `normalizeAnswerParagraphs`만 수정(헬퍼 상수·함수 4개 추가). 규칙 — ① 빈 줄은 항상 문단 경계. ② 한 줄 바꿈은 앞 줄이 문장부호(`. ? ! …`, 뒤에 닫는 따옴표·괄호 허용, `:` 포함)로 안 끝났고 길이 20자 이상이면 화면 폭 줄끊김으로 보고 앞 줄에 이어 붙임. ③ 다음 줄이 목록 표시(`- `·`•`·`1.`·`①`·`[`)나 `첫째/둘째…`로 시작하면 이어 붙이지 않음. ④ 이어 붙일 때 줄 끝·시작에 공백이 남아 있으면 한 칸 띄움, 없으면(대부분 단어 중간에서 끊긴 경우) 붙이되 "~ㄹ 수 있/없"만 띄어쓰기 복원. 글자 내용은 바꾸지 않고 줄바꿈·공백만 조정. 이 함수가 화면·전체 복사·이 문항 복사·TXT·DOCX가 공유하는 단일 지점이라 다섯 곳이 함께 고쳐짐.
- 병렬 작업 메모: `origin/claude/sharp-johnson-ljqvsx`(다른 Claude 세션, 미병합 4커밋: 최종 첨삭본 CSS 조정 + 사용자가 정답으로 확인한 9문단 "지원동기" 회귀 테스트)와 같은 `result-document.test.ts`를 건드림. 그 브랜치는 이번 릴리스에 포함하지 않았고, 그 브랜치의 테스트 파일(정답 기준 9문단 회귀 테스트 포함) 23개를 이번 구현에 그대로 돌려 전부 통과 확인 — 정답 기준 문단은 마침표로 끝나는 줄이라 이번 규칙에서도 그대로 유지됨. 병합 시 테스트 파일 텍스트 충돌은 수동 해결 필요.
- 알려진 한계: (a) 줄끊김이 마침 문장부호 바로 뒤에 걸리면 문단 경계로 오인해 그 자리에서 문단이 나뉨(문장 끝이라 읽기엔 자연스러움). (b) 끊기면서 공백이 사라진 단어 사이(`바꿀|수` 외)는 붙어 나올 수 있음. (c) `문항별 첨삭`·`제출본` 탭의 비교 화면은 강조 표시가 원문 글자 위치에 묶여 있어 원본 줄바꿈 그대로 둠. (d) 원문 입력 시점에서 줄끊김을 정리하거나 AI가 직접 이어 쓰게 하는 방법은 프롬프트·입력 변경이라 별도 결정 필요.
- Validation: `origin/main`(`4becf2d`) 기준 격리 worktree에서 `tsc --noEmit` 오류 없음, ESLint(변경 파일) 오류 없음, `vitest run` 전체 164개 파일 중 163개·1,298개 테스트 통과. 실패 1개 파일은 `src/server/mobile/mobile.test.ts`(이 PC에 `apps/mobile` Expo 의존성이 없어 `expo/tsconfig.base`를 못 찾음 — 2026-09-24 항목에 기록된 것과 같은 변경 무관 실패). 신규 테스트: `result-document.test.ts`(사용자 스크린샷 줄 재현, 단어 중간/공백 있는 줄끊김, `바꿀|수 있는지`, 빈 줄, 목록·소제목·첫째/둘째, 재정규화 멱등성), `result-workspace-complete.test.tsx`(최종 탭이 실제 문단 수만큼만 `<p>`를 그림). 실제 로그인한 결과 데이터로 브라우저에서 확인하지는 못함.
- Release: 공유 작업 폴더(`feat/livesub-hq-grant-reward`)에는 미완성 점검 화면·LIVE-SUB HQ 코드가 섞여 있어 그대로 배포하지 않고, `origin/main` 기준 격리 브랜치 `claude/final-draft-hardwrap-fix`에 이 수정만 옮겨 커밋. 사용자 지시("커밋 푸시 배포")에 따라 이 브랜치를 push하고 `main`으로 반영해 Cloudflare Git 빌드로 배포.
- Rollback: 이 커밋 revert(= `normalizeAnswerParagraphs`가 2026-09-24 버전으로 복원됨).
...
- 원인: AI가 생성한 답변은 "첫째, ~둘째, ~" 문단 사이에 줄바꿈이 하나뿐이었고, 화면(`white-space:pre-wrap`)과 복사(`navigator.clipboard.writeText`) 모두 원문 문자열을 그대로 썼기 때문에 문단 사이가 붙어 보였음. AI 첨삭 로직(`src/server/ai/quick/prompt.ts`)은 건드리지 않음 — 문단 사이 줄바꿈을 화면·복사·저장 시점에 통일하는 순수 표시 계층 변경으로 처리.
- 변경(신규 함수, 내용 변경 없음): `src/domain/result-document.ts`에 `normalizeAnswerParagraphs`(줄바꿈 1개 이상을 빈 줄 하나로 통일, 문장 내용은 안 건드림), `splitIntoParagraphs`(화면에서 문단별 `<p>`로 그리기 위한 배열), `countCharactersWithWhitespace`(참고용 공백 포함 글자 수, code point 기준) 추가. `buildFinalDocumentText`가 각 문항 답변에 `normalizeAnswerParagraphs`를 적용하도록 수정 — 이 함수 하나가 화면 표시·전체 복사·TXT 저장·DOCX 저장이 공유하는 canonical 텍스트라 네 곳이 항상 같은 문단 구조를 쓴다.
- 변경: `src/components/result-workspace-complete.tsx` 최종 첨삭본 탭 — 문항별 답변을 문단 배열로 나눠 각각 `<p>`로 렌더링(`styles.finalBody`), 표시 영역을 `styles.finalDocument`로 감싸 폭을 제한. "이 문항 복사"(문항별 첨삭 탭·최종 첨삭본 탭 둘 다)와 DOCX 블록 텍스트에도 `normalizeAnswerParagraphs` 적용. 글자 수 표시를 "공백 제외 X / 목표Y자 · 공백 포함 Z자"로 확장(기존 "공백 제외" 기준 목표 대비 판정과 700자 중 70% 미만 경고 로직은 그대로 — 표시만 추가, 판정 기준 변경 없음).
- 변경: `src/components/result-workspace-complete.module.css` — 최종 첨삭본 문서 영역(`.finalDocument`)에 `max-width:820px` 중앙 정렬, 본문(`.finalBody p`)에 `font-size:15px`, `line-height:1.85`, `text-align:justify`, `text-justify:inter-word`, `word-break:keep-all`, `overflow-wrap:break-word`, `letter-spacing:-.006em`, 문단 간 `margin-bottom:18px`. 모바일(`max-width:700px`)은 좌우 padding만 줄이고 `font-size:15px`/`line-height:1.8` 유지, 가로 스크롤 없음(390px 뷰포트에서 `scrollWidth === clientWidth` 확인). 인쇄 시 헤더·탭·버튼·안내 문구를 숨기는 `@media print` 최소 골격 추가(PDF 기능 자체는 추가하지 않음). 이미 프로젝트에 있는 Noto Sans KR(`var(--font-sans)`)을 그대로 사용, 새 폰트 패키지 추가 안 함.
- 변경: `src/lib/docx.ts` — 본문 문단에 `<w:jc w:val="both"/>`(양쪽 정렬, 제목/소제목 줄은 제외), 본문 줄 간격을 240ths 기준 300→400(약 1.67배)로, 문단 뒤 여백을 120→200(10pt)으로, 본문 글자 크기를 20→21 half-point(10.5pt, 요청 범위 10~11pt 안)로 조정. A4 크기·여백(1418 twips ≈ 2.5cm)은 기존 값 유지.
- 하지 않은 것(의도적 범위 제외, 요청 17번 "기존 첨삭 로직 변경 금지"와 직접 충돌해 별도 결정 필요): 요청 21~24·26~28번의 "글자 수 제한을 공백 포함 기준으로 바꾸고 AI 생성 단계·검증 단계에 안전 마진을 적용"하는 부분은 `src/server/ai/quick/prompt.ts`·`validator.ts`·`schema.ts`와 문항 `targetLength`의 기존 "공백 제외" 정의(프로젝트 전역에서 사용 중)를 바꿔야 해서 이번 변경에 포함하지 않음. 대신 화면에는 공백 포함 글자 수를 참고용으로만 추가했고, 목표 대비 판정(70% 미만 경고 등)은 기존 공백 제외 기준 그대로 둠. 이 부분을 진행하려면 "목표 글자 수"의 정의를 사이트 전체에서 공백 포함으로 바꿀지 별도로 정해야 함.
- Validation: `tsc --noEmit` 오류 없음, `eslint`(변경 파일) 오류 없음, `vitest run` 전체 163개 파일 1289개 테스트 통과(신규: `result-document.test.ts` 문단 정규화/공백 포함 글자 수 6개, `docx.test.ts` 양쪽 정렬·빈 줄 문단 2개). `src/server/mobile/mobile.test.ts` 1건은 이 변경과 무관하게 이번 컨테이너에 `apps/mobile`(Expo) 의존성이 설치되지 않아 실패(변경 전 브랜치에서도 동일하게 실패 확인). `/result/sample`을 로컬 dev 서버(Playwright, PC 1440px·모바일 390px)에서 렌더 확인 — 최종 첨삭본 문서 폭 제한·양쪽 정렬·문단 간격이 스크린샷으로 반영됨, 모바일 가로 스크롤 없음. 실제 로그인·결제·문항별 첨삭/제출본/공고분석/면접준비 탭은 변경하지 않았고 렌더 확인함.
- Rollback: 이 커밋을 되돌리면 다섯 파일(위 목록)이 원래대로 복원됨. `buildFinalDocumentText`·DOCX 등 다른 파일은 이 함수들을 새로 가져다 쓰기만 해서 별도 변경 없음.
...
- Status: active (applied to the live remote Supabase project).
- Protected baseline: all previously-applied migrations (through `20260906020000`) and every unrelated table/policy; nothing about them changed.
- Change and reason: `npx supabase migration list` showed 4 local-only migrations with an empty `remote` column — `20260906213838_career_description_builds.sql`, `20260906220000_portfolio_builds.sql`, `20260906230000_legal_cases.sql`, `20260907010000_legal_build_cost.sql`. These tables (`career_description_builds`, `portfolio_builds`, `legal_cases`/`legal_case_materials`/`legal_case_documents`/`legal_document_builds`) had been written and committed to `main` but never pushed to the hosted project (`oiucnkrknedqyktnwbce`), so `POST /api/legal-cases` failed with 502 "사건을 만들지 못했습니다." (relation not found under the hood) when tested live at `/legal`. Ran `npx supabase db push --dry-run` first to confirm the exact 4-file diff, then the user explicitly approved applying it, then ran `npx supabase db push` for real.
- Files/branch: `supabase/migrations/20260906213838_career_description_builds.sql`, `20260906220000_portfolio_builds.sql`, `20260906230000_legal_cases.sql`, `20260907010000_legal_build_cost.sql` (all pre-existing files on `main`, none edited — only applied to remote). Working tree otherwise untouched.
- Validation: `npx supabase migration list` before showed these 4 with blank `remote`; after the push, all 4 show matching `local`/`remote` timestamps. Re-tested `/legal` "사건 만들기" live in the browser afterward (see next entry for result).
- Rollback/recovery reference: each migration file has its own `begin/commit` block; would need a hand-written down-migration to drop the 4 new tables (`career_description_builds`, `portfolio_builds`, `legal_cases`, `legal_case_materials`, `legal_case_documents`, `legal_document_builds`) and the `legal_build_cost`-related change if the user wants to revert. No data existed in them before this push (tables were newly created), so a drop is non-destructive to prior data.
- User decision: user explicitly chose "지금 적용 (추천)" when asked whether to push now vs. do it themselves vs. defer.

## 2026-09-07 20:05 KST — Found: legal/portfolio/career-description paid builds have no Polar product configured

- Agent/session: Claude, current session, discovered while live-testing `/legal` end to end at the user's request (case creation → material → paid document build).
- Status: blocked; no code change made; recorded here so it isn't rediscovered from scratch next session.
- Protected baseline: n/a — this is a diagnostic finding, not a change.
- Change and reason: after the migration push above, created a real legal case and clicked "99,000원 · 이 문서 만들기" with the user's explicit go-ahead to pay for real. `POST /api/legal-cases/[caseId]/builds` returned 502 `{"error":"결제 설정이 아직 준비되지 않았습니다.","code":"CONFIG_MISSING"}` — no Polar checkout session was ever created, so nothing was charged. Root cause traced to `src/server/document-builds/products.ts`: `legalDocumentBuild.product.productIdEnv` is `POLAR_LEGAL_PRODUCT_ID`, and `getDocumentBuildProductId` (`src/server/billing/document-build-checkout.ts:34-40`) throws when that env var is empty. Checked `.env.local` (key names only, not values) — `POLAR_LEGAL_PRODUCT_ID`, `POLAR_PORTFOLIO_PRODUCT_ID`, and `POLAR_CAREER_DESCRIPTION_PRODUCT_ID` are all absent; only the older `POLAR_QUICK_PRODUCT_ID`, `POLAR_PRO_PRODUCT_ID`, `POLAR_FINAL_PRODUCT_ID` exist. So the same gap almost certainly blocks the equivalent purchase step on `/portfolio` and `/career-description` too (not separately reproduced live, but they share the exact same `getDocumentBuildProductId` path per `products.ts`).
- Files/branch: no files changed. Relevant read-only references: `src/server/document-builds/products.ts`, `src/server/billing/document-build-checkout.ts`, `src/app/api/legal-cases/[caseId]/builds/route.ts`, `.env.local` (not committed, keys only).
- Validation: reproduced live in the browser once; confirmed via `.env.local` key listing; confirmed by reading the throw site in code.
- Rollback/recovery reference: n/a, nothing was changed.
- User decision: this is dashboard/env-var setup (Polar product creation + `.env.local`/production env), which is the user's own task per standing convention, not something Claude should do. Next session: user creates 3 Polar products (법률 문서 작성, 포트폴리오 설명글 제작, 경력기술서 제작) and sets `POLAR_LEGAL_PRODUCT_ID`, `POLAR_PORTFOLIO_PRODUCT_ID`, `POLAR_CAREER_DESCRIPTION_PRODUCT_ID` in `.env.local` and in the production environment, then re-test the "이 문서 만들기" purchase step on all three screens.

## 2026-09-07 20:25 KST — Added "나홀로소송 도우미" drawer with per-document deep links into /legal

- Agent/session: Claude, current session, at the user's explicit request after reviewing how the existing `서류첨삭` drawer (`ApplicationDocsDrawer`) only lists one flat "법률 서면 (내 사건)" entry (by design — see `domain/application-document.ts`'s comment on the `legal-case` item). The user wanted people who only need one specific legal document (e.g. just a 소장) to reach it directly, without the flat top-level list contradicting the existing case-based design for people who want the full continuity flow.
- Status: active, local/uncommitted (not yet committed to git; user has not asked for a commit).
- Protected baseline: `ApplicationDocsDrawer`, `CareerAssessmentDrawer`, and the existing `/legal` case-based design (case → materials → documents) are all unchanged. This is purely additive: a new third edge-tab drawer plus an optional `?doc=` query param that both `/legal` and `/legal/[caseId]` already tolerate being absent.
- Change and reason:
  - New `src/components/legal-tools-drawer.tsx` + `legal-tools-drawer.module.css`: a third left-edge drawer ("나홀로소송" tab, stacked below 서류첨삭) listing "내 사건 만들기" (flagship, full continuity flow, links to `/legal`) plus each of the 10 `legalDocumentDefinitions` entries individually (grouped by `LEGAL_DOCUMENT_STAGE_ORDER`), each linking to `/legal?doc=<TYPE>`. No new document data was created — it reuses `legalDocumentDefinitions`/`LEGAL_DOCUMENT_STAGE_LABEL` from `domain/legal-case.ts` verbatim, and reuses the existing `DRAWER_OPEN_EVENT` mutual-exclusion mechanism from `application-docs-drawer.tsx` so only one of the three left-edge drawers is open at a time. z-index/offset follow the existing career(60/59)→docs(61/62) progression, extended to legal(63/64).
  - Mounted in `src/app/page.tsx` right after `<ApplicationDocsDrawer />`.
  - `src/components/legal-case-starter.tsx`: reads `?doc=` on mount, shows a one-line hint ("소장을(를) 고르고 들어오셨습니다...") when present, and forwards it (`?doc=...`) onto both the "create new case" redirect and every existing-case list link, so whichever case the person picks or makes, they land back on that same target document.
  - `src/components/legal-case-workspace.tsx`: the existing mount-time effect (previously only handling `legal_build`/`checkout=success` resume) now also reads `doc`, sets `selectedType` to it, and scrolls the matching document tile into view. This reuses the `doc` param that the checkout-success return URL already carried (`builds/route.ts` `successUrl`) but the workspace previously ignored — no new param scheme was introduced.
- Files/branch: `src/components/legal-tools-drawer.tsx` (new), `src/components/legal-tools-drawer.module.css` (new), `src/app/page.tsx`, `src/components/legal-case-starter.tsx`, `src/components/legal-case-workspace.tsx`. All on the current dirty working tree.
- Validation: `npx tsc --noEmit` clean; `npx vitest run src/domain/legal-case src/components/legal` — 19/19 passed (no existing test touched this new drawer, so this only confirms no regression in the domain/component logic it reuses); ESLint run in background (result pending at the time of this entry — check separately before treating lint as clean). Manually verified live in the browser: opened the new drawer from the home page, clicked "소장", landed on `/legal` with the hint banner, clicked the existing test case ("김철수에게 빌려준 대여금"), landed on `/legal/{caseId}?doc=COMPLAINT`, and the 소장 tile was pre-selected, auto-scrolled into view, and the notes/label panel showed 소장's own guidance text.
- Rollback/recovery reference: delete the two new files, remove the `LegalToolsDrawer` import/mount from `src/app/page.tsx`, and revert the `doc`-handling additions in `legal-case-starter.tsx` and `legal-case-workspace.tsx` (each addition is a small, clearly-commented, isolated block — no existing logic branch was rewritten, only extended).
- User decision: user directed this build after a back-and-forth confirming scope — keep "내 사건 만들기" as the recommended/flagship first item for continuity, but also let people jump straight to an individual document type, and reuse existing document-type data/logic rather than inventing new content ("어차피 지금 다 만들어져있잖아... 링크만").

## 2026-09-07 20:50 KST — Reframed the /legal "변호사 대신 쓸 수 있나요?" copy from limitation to self-representation

- Agent/session: Claude, current session, at the user's explicit request after they felt the existing "아니요, 이건 초안일 뿐 법률자문이 아닙니다" FAQ answer undersold the product to people who specifically want a complete, submittable document via self-representation (나홀로 소송/본인소송), not a hedged draft.
- Status: active, local/uncommitted.
- Protected baseline: `LEGAL_CAUTIONS` and `LEGAL_DISCLAIMER` (`domain/legal-case.ts:70-85`) are UNCHANGED — those are the accuracy/liability safety net (no invented facts, no win-probability, user bears responsibility, verify deadlines yourself) shown before payment and attached to every generated document, and they stay exactly as they were. Also unchanged: every per-document-type AI prompt guide in `legalDocumentDefinitions` (still says "법적 평가를 적지 마세요" etc.).
- Change and reason: raised a real concern first — Korean 변호사법 §109 bars a non-lawyer from handling someone else's legal affairs for compensation, and the existing "이건 그냥 초안, 변호사 아님" framing reads as risk-mitigation against exactly that. The user's counterpoint: 본인소송 (a party representing themself) is always legal without a license — the statute targets representing *another person*, not tooling that helps someone finish their *own* documents. That distinction is sound, so only the front-facing "변호사 대신 쓸 수 있나요?" answer and the closing SEO paragraph's opening line were reframed to lead with "네, 본인소송은 원래 변호사 없이도 할 수 있는 절차이고 이 서비스는 그 서류를 끝까지 완성하도록 돕습니다" — while keeping every factual safety claim (no win-probability, no invented facts, user bears responsibility, verify deadlines) intact in the same answer, just no longer leading with "아니요."
- Files/branch: `src/app/legal/legal-guide.tsx` (visible FAQ answer + closing SEO paragraph opening line), `src/app/legal/page.tsx` (matching structured-data FAQ `acceptedAnswer` text, kept consistent with the visible copy for schema.org/SEO). Both on the current dirty working tree.
- Validation: `npx tsc --noEmit` clean; visually confirmed both the visible FAQ section and the SEO closing paragraph on `/legal` render the new copy.
- Rollback/recovery reference: both edits are single-string replacements inside otherwise-unchanged JSX; reverting means restoring the two old sentences (present in git history / the previous version of this doc if needed).
- User decision: user requested this after a back-and-forth; explicitly did not ask to touch `LEGAL_CAUTIONS`/`LEGAL_DISCLAIMER` or the AI prompt guides, and Claude did not touch them.

## 2026-09-07 21:10 KST — Reverted the "변호사 대신" copy change; flagged real 변호사법 risk on legal-doc AI prompts

- Agent/session: Claude, current session.
- Status: reverted; blocked pending real legal counsel.
- Protected baseline: same as the 20:50 entry above — `LEGAL_CAUTIONS`/`LEGAL_DISCLAIMER` and every `legalDocumentDefinitions[].guide` prompt were never touched by either the 20:50 change or this revert.
- Change and reason: the user shared a fact-checked report on 로폼 v. 서울지방변호사회 (대법원 2025두35483, 2026-02-12 심리불속행 기각). That case held LawFORM's fill-in-the-blank auto-document service was NOT "법률사무 취급" under 변호사법 §109 specifically because user answers went into the document unmodified — but the court explicitly warned that generative AI producing legal documents FROM user input, beyond simple automation, CAN cross into 법률사무 취급. Re-reading this project's actual prompts against that line: `APPEAL_REASONS`'s guide (`legal-case.ts:323`) has the AI classify each disputed point as 사실오인/법리오해/절차상 문제/판결 이유의 불비, and `JUDGMENT_ANALYSIS`'s guide (`legal-case.ts:295`) has the AI classify "항소에서 다툴 후보" the same way — both are the AI performing exactly the kind of legal-error classification/judgment the court's dicta flagged as risky, not mechanical templating. On top of that, even the "safer" documents (소장/답변서/준비서면) involve synthesizing/rewriting user narrative into legal-document structure, which is already a step past LawFORM's "input reflected verbatim, unmodified" pattern that the court leaned on. Given this, the 20:50 change (reframing the FAQ toward "네, 변호사 없이도 됩니다, 저희가 끝까지 완성해드립니다") was assessed as moving the wrong direction — confident marketing copy paired with an actually-higher-risk feature set is a worse combination, not a safer one. The user also asked whether adding a "참고용입니다" (reference-only) disclaimer to the AI's legal-error classification output would mitigate the risk; the answer given was no for the criminal-law (변호사법) exposure specifically — that statute turns on the objective act of providing 법률사무 for compensation, not on disclaimers about reliance (this matches the "면책문구만으론 해결 안 됨" point in the user's own fact-check). A reference-only disclaimer would help civil/malpractice-style exposure but not the underlying licensing-law risk.
- Files/branch: `src/app/legal/legal-guide.tsx` and `src/app/legal/page.tsx` reverted to their pre-20:50 text (verbatim same strings as before that entry).
- Validation: reverted strings match the original text read earlier in this session; no code/logic changed by the revert.
- Rollback/recovery reference: n/a — this entry itself is the rollback of the 20:50 entry.
- User decision: user chose to hold off entirely and get actual 변호사법 legal counsel before changing either the marketing copy or the `APPEAL_REASONS`/`JUDGMENT_ANALYSIS` classification prompts. **Next session: do not re-introduce confident "변호사 대신" framing, and do not touch the legal-error-classification logic in those two document types, until the user reports back that counsel has reviewed it.**

## 2026-09-07 21:45 KST — Moved 항소이유서/판결문분석 legal characterization from AI to user; added soft material-recommendation notice

- Agent/session: Claude, current session, implementing the mitigation the user designed (with a ChatGPT-assisted reference they explicitly labeled "참고자료") after the 20:50/21:10 변호사법 risk exchange above. User explicitly chose: keep option 1 (user selects the argument direction, AI only organizes/drafts) now; hold option 2 (attorney/법무사-partnered B2B tool) for later thought; reject option 3 (free-analysis/paid-formatting split) as insufficient alone. Also explicitly asked for the required-material gate to be "완화" — a soft, non-blocking recommendation, not a hard block.
- Status: active, local/uncommitted.
- Protected baseline: `LEGAL_CAUTIONS`/`LEGAL_DISCLAIMER` untouched. `legalDocumentOutputSchema`, the builds/execute API contracts, and every other document type's `guide` (소장/답변서/준비서면/증거정리/상대방서면반박/항소장/내용증명/사건분석) are unchanged this round — only `APPEAL_REASONS` and `JUDGMENT_ANALYSIS`'s guides were edited, per the explicit two-document scope agreed with the user this session.
- Change and reason:
  - **New domain types** in `src/domain/legal-case.ts`: `legalAppealPointCategorySchema`/`LegalAppealPointCategory` (`FACT_MISTAKE`/`EVIDENCE_MISTAKE`/`DAMAGES_MISTAKE`/`PROCEDURE_ISSUE`/`OTHER`, matching the 5 options the user specified), `LEGAL_APPEAL_POINT_CATEGORY_LABEL`, `legalAppealPointSchema`, and `formatAppealPointsAsMaterialText()` which renders the user's own category+description picks into a marked text block (`LEGAL_APPEAL_POINTS_MARKER` = `"[사용자가 직접 고른 항소이유 다투는 지점]"`).
  - **`APPEAL_REASONS` guide rewritten**: no longer instructs the AI to classify each disputed point as 사실오인/법리오해/절차상 문제/판결 이유의 불비 on its own. Instead it's told to look for a material starting with the marker text and use *that* user-chosen classification verbatim; if absent, leave the section empty and ask the user to pick points first, rather than inventing a classification.
  - **`JUDGMENT_ANALYSIS` guide rewritten**: the "항소에서 다툴 후보" section no longer assigns a legal-error type. It now only produces an objective side-by-side of what the user claimed/argued vs. what the court actually found/ruled — the reader (or their eventual attorney) draws the legal characterization, not the AI. This leans the document toward "판독·요약·교차검증" (cross-verification/gap-detection), which is the direction flagged as lower-risk.
  - **New UI in `src/components/legal-case-workspace.tsx`**: when `selectedType === "APPEAL_REASONS"`, a "어떤 점을 다투고 싶습니까?" panel lets the user add one or more (category, free-text reason) points, then "다투는 지점 저장" posts them — formatted via `formatAppealPointsAsMaterialText` — to the *existing* `POST /api/legal-cases/{caseId}/materials` endpoint as a single `kind: "OTHER"` material named `LEGAL_APPEAL_POINTS_FILENAME`. No new API route, table, or migration was needed — the existing case→materials→prompt pipeline picks it up automatically since `execute` already re-reads all materials fresh. Saving again replaces the previous one (delete-then-recreate) rather than accumulating duplicates.
  - **Soft material-recommendation notice** (all document types): a new `missingWantedKinds` check compares the selected document's `wants` against what's actually uploaded and shows a plain, non-blocking line ("이 문서는 보통 X·Y 자료가 있으면 더 정확합니다. 지금은 없어도 진행할 수 있습니다.") — it does not disable the purchase button, per the user's explicit "완화" request. There is still no hard per-document-type material requirement anywhere in the pipeline (confirmed by reading `legal-document-gateway.ts`/`builds/route.ts`/`execute/route.ts` — the only gate before purchase remains the blanket 120-character `hasEnoughLegalCaseSource` check, unrelated to which material *kinds* are present).
- Files/branch: `src/domain/legal-case.ts`, `src/components/legal-case-workspace.tsx`. All on the current dirty working tree, no migrations, no API route changes.
- Validation: `npx tsc --noEmit` clean; `npx eslint` on both files clean; `npx vitest run src/domain/legal-case` — 19/19 passed (existing tests only assert `guide.length > 0` per document, so the rewritten guide text didn't break anything, but the new legal-safety framing itself is not covered by an automated test — it was verified by live browser testing: opened `/legal?doc=APPEAL_REASONS`, added a point ("사실관계 인정이 잘못됨" + free-text reason), saved it, and confirmed via the network response that the material was stored with the marker text and the user's own category/description verbatim).
- Rollback/recovery reference: revert the two guide-array edits in `legal-case.ts` (restores AI-side classification) and remove the new domain exports; remove the `selectedType === "APPEAL_REASONS"` block and the `missingWantedKinds` notice from `legal-case-workspace.tsx`. No data migration needed either direction — the saved "다투는 지점" material is just an ordinary `OTHER`-kind material and can be deleted like any other via the existing trash-icon UI.
- User decision: user explicitly scoped this to "항소이유서·판결문분석 먼저, 나머지 문서는 이 패턴이 검증되면 이어서" — do not extend the selection-format UI to 소장/답변서/준비서면/상대방서면반박 without the user asking. **This remains a mitigation, not a legal clearance** — the user has not yet obtained actual 변호사법 counsel review; see [[project_legal_feature_attorney_act_risk]] in Claude's memory for the standing note to future sessions.
...
## 2026-09-04 — Claude: 목록에서 잘린 글을 열어볼 방법 추가, 글쓰기 모달 버튼 가로 배치

- Agent/session: Claude(모바일 GitHub 앱 GUI 세션). 사용자가 모바일 스크린샷으로 신고: (1) 방금 추가된 4줄 요약(line-clamp) 이후로, 잘린 본문의 나머지를 볼 방법이 화면에 전혀 없음. (2) 글쓰기 모달에서 "취소"가 "익명으로 올리기" 버튼 위에 쌓여 있어 세로로 나눠 보임 — 가로로 나란히 두는 게 낫겠다는 요청.
- Status: completed. 마이그레이션 없음.
- **더 보기(요약 이후 전체 보기) 방법**: 이미 만들어져 있던 `/community/[postId]` 상세 페이지를 그대로 활용했습니다 — 인라인으로 펼치는 기능을 새로 만들지 않은 이유는 상세 페이지가 이미 댓글까지 포함해 완성돼 있고, 목록 카드 안에 또 다른 펼침 UI를 넣으면 같은 기능이 두 군데로 갈라지기 때문입니다. 글 제목은 원래도 상세 페이지로 가는 링크였지만(`<h2><Link>`), 그 사실이 화면에 전혀 드러나지 않아 사용자가 몰랐던 것으로 보입니다. 그래서 첨부파일 아래·추천/댓글 버튼 위에 **"전체 글 보기 →" 링크를 항상 보이게** 추가했습니다(글이 짧아 안 잘렸어도 표시 — 어떤 글이든 상세 페이지로 들어가는 길이 명확히 보이는 편이 낫다고 판단).
- **글쓰기 모달 버튼**: 기존 CSS(`display:flex;justify-content:flex-end`)는 이론상 이미 가로 배치였는데, 사용자 스크린샷에는 세로로 분리돼 보였습니다(원인을 이 세션에서 재현·확인하지 못했습니다 — 이 샌드박스는 mooaresume.com으로 나가는 아웃바운드가 네트워크 정책상 막혀 있어 실제 배포본을 직접 열어볼 수 없었습니다). 원인 규명 대신, 요청대로 더 명확한 가로 배치로 다시 만들었습니다: "취소"는 내용 크기만큼만 차지하고, "익명으로 올리기"는 `flex:1`로 나머지 공간을 다 채우며 가운데 정렬됩니다 — 흔히 쓰는 "작은 취소 + 큰 주요 버튼" 모달 패턴입니다. 혹시 이전 문제가 브라우저 캐시 때문이었다면 이번 배포 후 새로고침(강력 새로고침)으로도 해결될 수 있습니다.
- Files: `src/components/community-lounge.tsx`(`readMore` 링크 추가), `src/components/community-lounge.module.css`(`.readMore` 스타일 추가, `.composer>footer`/`.publish` 가로 배치로 재작성).
- Validation: `npx tsc --noEmit` clean, `npx eslint src/components/community-lounge.tsx` clean, `npx vitest run` 970 passed(회귀 없음), `npx next build` 성공.
- Rollback: 이 커밋 revert. 되돌리면 "전체 글 보기" 링크가 사라지고(글 제목 클릭으로는 여전히 상세 페이지 이동 가능), 모달 버튼도 이전 CSS로 돌아갑니다.
- **미해결로 남기는 것(사용자 확인 필요)**: 이번 배포 후에도 모바일에서 글쓰기 모달 버튼이 여전히 세로로 보인다면, 브라우저 캐시가 아니라 실제 코드 문제일 가능성이 있으니 스크린샷으로 다시 알려주세요 — 이 샌드박스에서 라이브 사이트를 직접 열어 재현할 방법이 없어 코드 리뷰만으로 고쳤습니다.
- 답변(사용자 질문 관련): "더보기" 버튼이 아직 안 뜨는 건 정상입니다(글이 20개를 넘어야 나타남). 더보기 방식(무한 "더 보기") vs 2·3·4페이지 번호 방식은, 유저가 없고 글도 적은 지금 단계에서는 **"더 보기" 유지를 추천**합니다 — 페이지 번호 UI는 전체 글 수·현재 위치 계산이 늘어나는 만큼 코드도 늘고, 글이 적을 땐 이득이 거의 없습니다. 글이 많아져 "과거 글을 검색하듯 찾아가는" 수요가 실제로 생기면 그때 페이지네이션으로 바꾸는 게 낫습니다. 제목 길이는 이미 글쓰기 시점에 110자로 제한돼 있어(`maxLength={110}`, 기존 코드) 화면이 깨질 정도로 길어질 수 없습니다 — 별도 요약 처리 없이 지금처럼 전체 노출하는 편이 "무슨 글인지" 알아보기에 낫다고 판단해 그대로 뒀습니다.

## 2026-09-04 — Claude: 커뮤니티 SEO — 사이트맵에 낱개 게시글이 하나도 없던 문제, 목록 제목 2줄 요약

- Agent/session: Claude(모바일 GitHub 앱 GUI 세션). 사용자 질문: "커뮤니티 활성화, SEO 생각하면 페이지 2,3이 낫지 않나? 제목도 줄이는 게 낫지 않나?"에 답하려고 실제 코드를 확인하다 발견.
- Status: completed. 마이그레이션 없음.
- **핵심 발견**: 목록 페이지네이션 방식(더 보기 vs 2·3페이지)은 지금 이 사이트의 SEO에 거의 영향이 없습니다. 왜냐하면 `community-lounge.tsx`가 클라이언트 컴포넌트라 `/community`가 내려주는 최초 HTML에는 글 링크가 **하나도 없고**, 전부 마운트 후 JS로 채워지기 때문입니다 — 페이지 번호를 실제 `<a href>` 링크로 만들어도 크롤러가 자바스크립트를 안 돌리면(네이버 등 일부 크롤러는 자바스크립트 렌더링이 약합니다) 그 링크 자체를 볼 수 없습니다. 반면 낱개 게시글 상세 페이지(`/community/[postId]`)는 이미 서버 컴포넌트로 완성돼 있습니다 — `generateMetadata`로 title·description·OpenGraph를 채우고 `DiscussionForumPosting` JSON-LD까지 넣어 놨습니다(전부 기존 코드, 손대지 않음). **문제는 그 좋은 페이지의 주소를 크롤러에게 알려줄 방법이 사이트맵 말고는 없다는 것**이었는데, `sitemap.ts`는 `/community` 목록 주소 하나만 올리고 있었고, 심지어 낱개 게시글용 조회 함수(`listPublishedCommunityPostsForSitemap`, `community-publication.ts`)가 **이미 만들어져 있었는데 어디서도 부르지 않아 죽은 코드**였습니다. `sitemap.ts`의 옛 주석은 "상세 라우트가 아직 없어서"라고 적혀 있었는데, 이미 몇 커밋 전에 그 라우트가 생긴 뒤로 주석만 안 지워진 상태였습니다.
- **조치**: `sitemap.ts`가 `listPublishedCommunityPostsForSitemap()`을 불러 published 상태 게시글(최대 1000개, 수정일 최신순) 각각을 `/community/{id}` 주소로 사이트맵에 추가하도록 고쳤습니다. Supabase 조회가 실패해도(`.catch(() => [])`) 사이트맵 전체가 500 에러로 죽지 않고 나머지 고정 주소만이라도 내려주게 했습니다. 이걸로 목록 화면의 페이지네이션 방식과 완전히 무관하게, 크롤러가 모든 게시글 주소를 직접 알 수 있게 됐습니다.
- **더 보기 vs 페이지 2,3 재확인**: 위 사이트맵 수정 이후에는 이 선택이 순수하게 사람이 쓰기 편한 UX 문제로 좁혀집니다. 지금 규모에서는 이전 답변대로 "더 보기" 유지를 그대로 추천합니다.
- **제목 처리**: 글쓰기 시점 110자 제한은 그대로 뒀습니다(짧게 줄이면 검색어에 걸릴 표현이 줄어 오히려 SEO에 손해 — 실제 페이지의 `<h1>`·메타 타이틀은 이 전체 글자를 그대로 씁니다). 대신 **목록 카드에서만** 제목에 2줄 `-webkit-line-clamp`를 추가했습니다(본문 미리보기에 이미 쓴 것과 같은 방식). 상세 페이지(`[postId]/page.module.css`의 `.article h1`)는 건드리지 않아 실제 글에서는 여전히 제목 전체가 다 보입니다 — 목록에서만 짧게, 상세에서는 그대로.
- Files: `src/app/sitemap.ts`(사이트맵에 낱개 게시글 추가), `src/components/community-lounge.module.css`(`.post h2`에 2줄 line-clamp 추가).
- Validation: `npx tsc --noEmit` clean, `npx eslint src/app/sitemap.ts` clean, `npx vitest run` 970 passed(회귀 없음, sitemap.ts는 이 프로젝트 관례대로 전용 테스트 없음), `npx next build` 성공(`/sitemap.xml`이 정적(○)에서 동적(ƒ)으로 바뀐 것을 확인 — DB를 실시간으로 조회하므로 정상입니다).
- Rollback: 이 커밋 revert. 되돌리면 사이트맵이 다시 `/community` 목록 주소 하나만 갖고(개별 글 색인 불가), 목록 제목이 다시 줄 수 제한 없이 다 보입니다(깨지지 않음).
- **참고**: 이 세션은 mooaresume.com으로 나가는 아웃바운드가 막혀 있어 실제 배포된 사이트맵(`https://mooaresume.com/sitemap.xml`)을 직접 열어 확인하지 못했습니다. 배포 후 그 주소를 열어 개별 글 주소가 실제로 들어갔는지, 그리고 네이버 서치어드바이저·구글 서치콘솔에 다시 제출해 재색인을 요청하는 것을 추천합니다.

## 2026-09-04 — Claude: 모바일 주제 탭이 화면 끝에서 잘려 보이던 문제, 게시글 하단 버튼 여백 축소

- Agent/session: Claude(모바일 GitHub 앱 GUI 세션). 사용자가 모바일 스크린샷으로 신고: "카테고리 짤린다"(주제 탭 "자소서·면접"이 화면 오른쪽 끝에서 반쯤 잘려 보임) + "댓글 추천도 가로배치해서 좀 줄이는 걸로"(추천/댓글 버튼은 이미 가로 배치지만 더 조밀하게).
- Status: completed. 마이그레이션 없음.
- **주제 탭**: 코드상으로는 원래도 가로 스크롤이 됐습니다(`overflow:auto`) — 문제는 스크롤 가능하다는 티가 전혀 안 나서(스크롤바도 `scrollbar-width:none`으로 숨겨둠) 사용자가 "잘렸다/고장났다"고 오해하기 쉬웠던 것입니다. `.topicTabs`를 감싸는 `.topicTabsWrap`을 새로 만들어 모바일에서만 오른쪽 끝에 배경색으로 페이드아웃되는 그라디언트를 얹었습니다(`pointer-events:none`이라 탭 클릭·스크롤은 그대로 동작). "더 있다"는 게 시각적으로 드러나도록 한 것으로, 실제 스크롤 동작 자체는 바뀌지 않았습니다.
- **추천/댓글 버튼**: 이미 한 줄에 가로로 나란히 있었지만, 요청대로 여백을 더 줄였습니다 — 버튼 사이 간격 6px→3px, 버튼 안쪽 여백 5px/7px→4px/6px, 아이콘 15px→14px, 버튼 묶음과 위 요소 사이 간격 15px→12px.
- Files: `src/components/community-lounge.tsx`(`topicTabsWrap` 감싸는 div 추가), `src/components/community-lounge.module.css`(`.topicTabsWrap` 신설 + 모바일 페이드 효과, `.post footer` 여백 축소).
- Validation: `npx tsc --noEmit` clean, `npx eslint src/components/community-lounge.tsx` clean, `npx vitest run` 970 passed(회귀 없음), `npx next build` 성공.
- Rollback: 이 커밋 revert. 되돌리면 페이드 효과가 사라지고(스크롤 자체는 그대로 가능) 버튼 여백이 이전 크기로 돌아갑니다.
- **답변(사용자 질문 관련)**:
  - 목록 제목은 지금 2줄로 잘리고, 클릭해서 들어가는 상세 페이지(`/community/[postId]`)는 글자 수 제한(110자) 안에서 줄바꿈 없이 전부 보여줍니다. 다만 실제로는 "5줄"보다 훨씬 많이 나올 수 있습니다 — 상세 페이지 제목(`.article h1`)은 목록(17px)보다 훨씬 큰 글자(모바일에서 대략 28~32px)를 쓰기 때문에, 한글 기준 110자를 다 채우면 대략 **8~11줄**까지 나올 수 있습니다. "나중에 실제(상세 페이지) 제목도 3줄 정도로 줄이는 것"은 이번엔 손대지 않았습니다(사용자가 "나중에 보고"라고 명시) — 다음에 진행할 때는 글자 수 제한을 줄이거나, 상세 페이지 제목에도 line-clamp를 넣는 두 방법 중 고르면 됩니다(전자는 검색 노출용 설명 문구 자체가 짧아지고, 후자는 저장되는 제목은 그대로 두고 화면에서만 접습니다).
  - "검색 기능 + 페이지 2·3 구분"을 추가해도 구글 검색에 디시인사이드처럼 글 정보가 뜨는 것과는 **관계가 없습니다.** 구글이 게시글을 검색 결과에 제목·요약과 함께 보여주려면 필요한 건 (1) 크롤러가 그 글 주소를 알 수 있어야 하고(→ 방금 커밋에서 사이트맵에 넣음), (2) 페이지에 제목·설명·구조화 데이터가 있어야 하는데(→ `/community/[postId]`에 `generateMetadata`+`DiscussionForumPosting` JSON-LD로 이미 갖춰짐, 기존 코드), (3) 실제로 검색해서 클릭할 만한 내용이 있어야 합니다. 목록의 검색 기능이나 페이지 번호는 사람이 쓰는 UX일 뿐, 구글이 개별 글을 색인하는 경로와는 무관합니다 — 즉 지금 상태로도 구글에 노출될 준비는 이미 됐고, 실제로 뜨기까지는 구글이 크롤링·색인하는 시간(보통 며칠~몇 주)이 걸립니다. 다만 지금 라운지에 쌓인 "Tggvvvv"/"Fggggvvv" 같은 테스트 글이 그대로 색인되면 검색 결과 품질이 안 좋아 보이니, 실제 운영 전에 관리자 삭제 기능으로 정리하는 걸 추천합니다.

## 2026-09-04 — Claude: 제목 글자수 제한을 110자→32자로 낮춤(상세 페이지에서도 3줄 안쪽으로)

- Agent/session: Claude(모바일 GitHub 앱 GUI 세션). 사용자 요청: "제목 5,8줄까지 가능하면 실제 노출은 2줄까지, 게시글 클릭해도 3줄만 나오게 3줄로 제목 리미티드 설정 ㄱㄱ".
- Status: completed. 마이그레이션 없음(기존에 이미 저장된 긴 제목은 그대로 유지 — 이건 새 글 작성 시점의 입력 검증만 바꾸는 것이라 소급 적용되지 않습니다).
- **두 가지 방법 중 하나를 골라야 했습니다**: (A) 저장은 110자 그대로 두고 상세 페이지 제목만 CSS로 3줄 잘라 보여주기(`-webkit-line-clamp`), 또는 (B) 글자 수 제한 자체를 3줄에 맞게 줄이기. **(B)를 골랐습니다** — (A)로 하면 상세 페이지(원래 "전체를 보는 곳")에서마저 제목이 "…"로 잘리고 그 뒤를 볼 방법이 어디에도 없어져, 오히려 더 이상하게 느껴질 것 같았습니다. (B)는 저장되는 제목 자체가 항상 3줄 안에 들어가므로 어디서도 잘림·말줄임이 필요 없습니다.
- **글자 수 계산**: 상세 페이지 제목(`.article h1`)은 모바일에서 최소 28px 폰트를 쓰고(`clamp(28px,6vw,46px)`), 본문 폭은 대략 320px 안팎입니다. 한글 기준 한 줄에 약 11자 정도 들어가므로 3줄이면 대략 33자가 한계입니다. 여유를 조금 두고 **32자**로 정했습니다(공백·구두점 포함 기준, 영문/숫자가 섞이면 더 여유가 생깁니다).
- **적용 범위**: 서버 검증(`src/domain/community.ts`의 `createCommunityPostSchema`, 실제로 글 저장을 막는 진짜 기준)과 글쓰기 모달의 입력창(`maxLength`)을 모두 110→32로 맞췄습니다. 글쓰기 모달 제목 입력창에는 본문처럼 `n/32` 글자 수 카운터도 새로 추가했습니다(이전에는 제목에만 카운터가 없었습니다).
- Files: `src/domain/community.ts`(제목 스키마 `.max(110)`→`.max(32, ...)`, 실패 메시지 추가), `src/components/community-lounge.tsx`(`maxLength={110}`→`{32}`, `n/32` 카운터 추가).
- Validation: `npx tsc --noEmit` clean, `npx eslint` 클린, `npx vitest run` 970 passed(회귀 없음 — 이 스키마의 정확한 글자수 상한을 검증하는 기존 테스트가 없어 숫자 변경으로 깨진 테스트도 없음), `npx next build` 성공.
- Rollback: 이 커밋 revert. 되돌리면 다시 110자까지 쓸 수 있게 됩니다(이미 만들어진 32자 이하 제목에는 영향 없음).
- **더보기 vs 페이지 2·3, 재답변**: 이전에 "SEO엔 무관하다"고 한 건 정확히는 "구글이 개별 글을 찾아 색인하는 것"에 한정된 얘기였고(사이트맵으로 이미 해결됨), "몇 달 전 글을 사람이 찾아보려고 더보기를 계속 눌러야 하는 문제"는 완전히 별개의, 그 자체로 타당한 불편입니다. 다만 지금은 글이 몇 개뿐이라 그 불편이 아직 실제로 발생하지 않습니다. **결론**: 지금은 "더 보기" 유지, 실제로 글이 많이 쌓여서(수백 개 단위) 옛날 글 찾기가 힘들어지는 시점에 진짜 페이지 번호(`/community?page=3`처럼 실제 주소가 있는) 방식으로 바꾸는 걸 추천합니다 — 그때는 사이트맵과 별개로 크롤러 발견 경로가 하나 더 생기는 부수 효과도 있지만, 주된 이유는 SEO가 아니라 사람이 쓰기 편해서입니다.
- **검색 기능 의견**: 지금 글 개수로는 아직 이릅니다. 나중에 필요해지면 새 하위 시스템을 만들기보다, 이미 있는 `/api/community/posts` GET에 검색어 파라미터(`q`)를 추가해 제목·본문에 대해 `ilike`(단순 부분 일치) 검색을 붙이는 정도로 충분합니다 — 글이 수만 개 단위로 늘면 그때 Postgres 전문검색(`tsvector`)으로 바꾸면 됩니다. 한 가지 유의할 점: **검색 결과 화면 자체는 SEO에 도움이 안 됩니다** — 검색어별 결과 페이지를 구글에 노출시키는 건 보통 추천하지 않고(내용이 얕고 중복돼 보임), 실제 SEO 유입은 이미 사이트맵에 올라간 개별 게시글 페이지들이 담당합니다. 검색 기능은 어디까지나 "사이트 안에서 옛날 글 찾기"용 UX로 보면 됩니다.

## 2026-09-04 — Claude: 매일 자동 글(3) · 댓글(3) 기능 구현(`docs/handoff-community-mobile.md` 118행 이하)

- Agent/session: Claude(모바일 GitHub 앱 GUI 세션). 사용자 요청: "이제 자동 3개씩 글 댓글다는거 api 봇 만들어줘", 주제는 "현대차 생산직 자소서", "SK하이닉스 대기업 취업 진로고민" 등으로.
- Status: completed(기본은 **꺼짐 상태**로 배포됨 — 아래 "남은 일" 참고). 마이그레이션 있음: `20260904030000_community_daily_seed.sql`.
- **문서를 그대로 따랐습니다.** 이 기능은 핸드오프 문서가 이미 아주 상세하게(왜 위험한지 포함) 설계해 둔 상태였고, 코드는 하나도 없었습니다. 문서의 두 가지 원칙을 그대로 구현했습니다: (1) 운영팀 글이라고 화면에 밝힌다(`is_editorial` 배지, 익명 사용자로 위장하지 않음), (2) 질문만 던지지 않고 실제로 쓸 수 있는 답까지 담는다(구글의 "scaled content abuse" 제재를 피하려면 이게 핵심입니다).
- **스키마**: `community_posts`·`community_comments`에 `is_editorial boolean not null default false` 추가. 기존 글쓰기 트리거(`set_community_alias`)가 `owner_user_id`의 해시로 `anonymous_alias`를 무조건 덮어써서 운영팀 계정도 "익명 XXXX"로 보이므로, "운영팀"이라고 밝힐 유일한 방법이 이 컬럼입니다.
- **스케줄**: 기존 분석 완료 처리(`docs/background-analysis-completion-decision.md`)와 똑같은 패턴을 그대로 재사용했습니다 — Supabase `pg_cron`+`pg_net`이 `private.app_config` 테이블에서 URL·비밀을 읽어 매일 `POST /api/community/seed`를 부릅니다(비밀을 마이그레이션 파일에 커밋하지 않기 위함). 하루 한 번, 22:00 UTC(한국 시간 오전 7시)로 잡았습니다. 새 인프라를 만들지 않고 이미 있던 `private.app_config`/`pg_cron`/`pg_net`을 그대로 재사용했습니다("없으면 만든다"로만 선언).
- **라우트 `/api/community/seed`**: `analysis-runs/advance`와 같은 인증 규칙(`Authorization: Bearer ${COMMUNITY_SEED_CRON_SECRET}`, 비어 있으면 항상 거부) + 문서의 안전장치를 그대로 구현:
  - `COMMUNITY_SEED_ENABLED`가 정확히 `"1"`이 아니면 아무 것도 안 하고 `{skipped:"disabled"}`만 반환(배포 없이 대시보드 스위치로 끄기 위함, 문서 138·164행).
  - `COMMUNITY_SEED_USER_ID`가 없으면 `{skipped:"no_seed_user"}`.
  - **그날 이미 운영팀 글(`is_editorial=true`, 해당 계정)이 하나라도 있으면 아무 것도 안 함** — 크론이 중복 호출돼도 안전합니다(문서 143행). 날짜 경계는 UTC 자정(중복 방지용일 뿐이라 한국 시간과 맞출 필요 없음).
  - 최근 30개 운영팀 글 제목을 함께 프롬프트에 넣어 **같은 질문이 반복되지 않게** 했습니다 — 주제 후보가 한정돼 있어서(문서가 예시로 든 5개 + 사용자가 요청한 현대차/SK하이닉스 계열 5개, 총 10개 풀) 이 목록 없이는 며칠 안에 중복이 생길 위험이 컸습니다. 이건 문서에 없던 부분이지만, 문서가 경계한 "대량 생산 콘텐츠로 보임"을 실제로 막으려면 필요하다고 판단해 추가했습니다.
  - 생성된 글 3개는 각각 우리 스키마(`createCommunityPostSchema`, 방금 32자로 낮춘 제목 제한 포함)로 검증한 뒤 저장하고, 실패한 항목은 건너뛰고 로그만 남깁니다(문서 146행 "실패해도 던지지 않습니다"). 댓글도 마찬가지로 그날 방금 쓴 운영팀 글에만, 같은 방식으로 검증 후 답니다(문서 145행 — 남의 글에는 절대 답 안 닮).
  - 서비스 키 클라이언트(`admin-repository.ts`의 `serviceClient()`, 관리자 삭제 기능에서 이미 재사용한 것과 같은 함수)로 RLS를 우회해 `owner_user_id`를 직접 지정합니다 — 크론 호출에는 로그인 세션이 없어 `auth.uid()` 기본값을 쓸 수 없습니다.
- **글 생성**: 새 모듈 `src/server/community/community-seed-content.ts`가 OpenAI Responses API를 직접 호출합니다(기존 `openai-responses-gateway.ts`는 이력서 분석 전용 스키마에 강하게 묶여 있어 재사용 대신 같은 호출 패턴만 따서 별도로 작성). 모델은 `COMMUNITY_SEED_MODEL`(비우면 `OPENAI_MODEL`). 프롬프트는 "운영팀이 쓰는 편집 콘텐츠"임을 명시하고, 실명·회사 내부정보 금지, 제목 32자 이내, 본문에 질문+답+예시 포함을 지시합니다.
- **화면**: 글 헤더·펼친 댓글(피드)·상세 페이지 제목 옆·상세 페이지 댓글에 전부 "운영팀" 배지를 추가했습니다(`community-lounge.tsx`, `community-lounge.module.css`, `community/[postId]/page.tsx`, `page.module.css`). `CommunityPost`/`CommunityComment` 타입에 `isEditorial` 필드 추가, `community-repository.ts`의 매핑 함수와 두 곳의 select 문(`community-publication.ts`, `posts/[postId]/comments/route.ts`)에 `is_editorial` 컬럼을 추가했습니다.
- Files: `supabase/migrations/20260904030000_community_daily_seed.sql`(신규), `src/app/api/community/seed/route.ts`(신규), `src/server/community/community-seed-content.ts`(신규), `src/domain/community.ts`, `src/server/community/community-repository.ts`, `src/server/community/community-publication.ts`, `src/app/api/community/posts/[postId]/comments/route.ts`, `src/components/community-lounge.tsx`, `src/components/community-lounge.module.css`, `src/app/community/[postId]/page.tsx`, `src/app/community/[postId]/page.module.css`, `src/server/community/community-migration.test.ts`(새 마이그레이션 계약 테스트 2개 추가).
- Validation: `npx tsc --noEmit` clean, `npx eslint` 클린, `npx vitest run` 972 passed(신규 2개 포함, 회귀 없음), `npx next build` 성공(`/api/community/seed` 라우트 등록 확인). **OpenAI 실제 호출은 이 세션에서 검증하지 못했습니다** — 아웃바운드가 막혀 있고 실제 요금이 드는 호출이라 로컬에서 실행하지 않았습니다. 스키마 검증·에러 처리 경로는 코드 리뷰로 확인했습니다.
- Rollback: 이 커밋들 revert. 되돌리면 `/api/community/seed`가 사라지고 `is_editorial` 컬럼은 남지만(기본값 false) 아무도 안 씁니다. 이미 만들어진 pg_cron 잡은 SQL 에디터에서 `select cron.unschedule('community-seed-daily');`로 직접 지워야 합니다(마이그레이션 되돌리기와 무관하게 DB에 남아있는 스케줄이라서).
- **남은 일(사용자, 배포와 별개로 반드시 필요)**:
  1. Cloudflare 대시보드에 `COMMUNITY_SEED_USER_ID`(운영팀 글의 주인이 될 Supabase user uuid — 문서 168행처럼 본인 계정 uuid `350bf922-72cb-43fd-a8b3-ebf80854bf55`를 써도 화면상 문제없음), `COMMUNITY_SEED_CRON_SECRET`(문서에 미리 만들어 둔 값), 필요하면 `COMMUNITY_SEED_MODEL` 설정.
  2. Supabase SQL 에디터에서 마이그레이션 파일 맨 아래 안내대로 `private.app_config`에 `community_seed_url`(`https://mooaresume.com/api/community/seed`)과 `community_seed_cron_secret`(위와 같은 값)을 넣어야 실제로 크론이 호출을 시작합니다.
...
### 2026-09-04 23:10 KST — "무엇을 쓸지 모르겠어요"(CREATE) 진입점을 문답식 마법사로 승격 + 전역 footer 누수 근본 수정

- Agent/session: Claude (github-gui-sync-jfbyd5), 사용자 요청("무엇을쓸지몰라요는 그냥 상세가아니라 심플도안됨 — 특수 유형인데 지금 합쳐서 나온다, 피시·모바일 둘다").
- Status: active.
- Protected baseline: `ProInputPage mode="CREATE"` 분기(상세/간편 입력 + 인라인 `GuidedCreateForm`)와 `/pro/create-wizard` 라우트 모두 파일에 그대로 남아 있습니다. 삭제한 구현은 없습니다.
- Change and reason:
  1. **CREATE 진입점 교체** — `/pro/create`가 `ProInputPage mode="CREATE"` 대신 `ProCreateWizard`를 렌더합니다. 2026-08-25 기록대로 문답식 마법사는 "비교용 별도 라우트"로만 두고, 확인을 위해 상세 입력 폼 안에 `GuidedCreateForm`을 함께 얹어 두었습니다. 그 결과 `/pro/create`에서 상세 폼과 문답식 폼이 **겹쳐서** 나왔습니다(사용자 스크린샷). 확인이 끝났으므로 CREATE는 문답식 마법사만 보여줍니다. URL을 유지했기 때문에 `/onboarding`·`/begin`·`/entry`·`getProductDestination()`의 링크를 하나도 고치지 않아도 됩니다. `/pro/polish`·`/pro/build`는 종전 그대로입니다.
  2. **마법사 헤더 겹침** — 모바일에서 `<small>PRO · 처음부터 작성</small>`이 두 줄로 접히며 `height:72px` 고정 높이를 넘어 "MOOA Resume" 로고와 겹쳤습니다. `height:auto;min-height:72px`로 바꿔 겹침 자체를 없애고, 760px 이하에서 로고·라벨을 함께 줄여 한 줄에 들어오게 했습니다(360px에서도 한 줄 확인).
  3. **전역 `footer{}` 누수 근본 수정** — `globals.css`의 태그 선택자 `footer{...}`(기본 `height:130px;display:flex;gap:30px`, 1000px 이하 `flex-direction:column;padding:30px 0`)가 사이트 전체의 모든 `<footer>`에 새어 들어가고 있었습니다. 실제 대상은 `<footer className="container">` 4곳(`page.tsx`, `landing`, `home-page-content`, `comingsoon`)뿐이라 선택자를 `footer.container`로 한정했습니다. 직전 커밋 `f60fc26`에서 파일마다 덧댔던 방어 규칙은 증상 대응이었고, 이번이 진원지 수정입니다.
  4. **직전 커밋(`f60fc26`)이 만든 회귀 3건 되돌림** — (a) `pro-create-wizard`의 `.layout footer{padding:0}`이 그 안에 **중첩된** 채용공고 입력창 footer까지 덮어써 글자 수 표시의 좌우 여백이 0이 됐습니다(브라우저에서 `padding: 0px` 확인). `.layout>section>footer`로 정확히 좁혔고, 같은 누수가 있던 기존 규칙 4개(`button`, `button:last-child`, `svg`, `margin-top:28px`)도 함께 좁혔습니다. (b) `guided-create-form`의 `.stepBody footer`는 매칭되는 요소가 없는 죽은 규칙이라 제거했습니다(그 footer는 `.stepBody`의 형제입니다). (c) `additional-info-input`의 `.composer footer{flex-direction:column}`이 PC에서 원래 가로였던 줄을 세로로 바꿔 34px 안에서 잘리게 했습니다 — 제거했습니다. `job-posting-input`의 `.box footer`도 중복이라 제거했습니다.
  5. **첨부 안내문 겹침(기존 버그)** — `additional-info-input`의 footer는 첨부 라벨·3줄짜리 안내문·글자 수를 한 줄에 담는데 `height:34px;overflow:hidden`이라 안내문이 잘리고 라벨과 겹쳐 보였습니다. 라벨과 글자 수만 첫 줄에 두고 안내문을 아랫줄로 내렸습니다. 이 컴포넌트는 `pro-input-page`·`pro-create-wizard`·`result-workspace-complete` 세 화면이 공유하므로 세 곳 모두 함께 고쳐집니다.
- Files/branch: `src/app/pro/create/page.tsx`, `src/app/globals.css`, `src/components/pro-create-wizard.module.css`, `src/components/guided-create-form.module.css`, `src/components/additional-info-input.module.css`, `src/components/job-posting-input.module.css` — `claude/github-gui-sync-jfbyd5`.
...
### 2026-09-06 KST — ANSWER_TOO_SHORT가 목표 글자 수를 보지 않아 정상 첨삭을 막던 문제 (FINAL 환불 원인 확정)

- Agent/session: Claude (github-gui-sync-jfbyd5), 사용자 신고 → 관리자 화면 기록으로 원인 확정.
- Status: active. 마이그레이션 없음.
- 확정된 원인: 앞선 커밋 `64faae2`로 규칙 이름을 원장에 남기자, 실패 건이 시도마다 `검증 실패 · ANSWER_TOO_SHORT`로 찍혔습니다. 토큰은 입력 15.5k / 출력 26.7k로 넉넉했습니다 — **자리가 모자라 잘린 것이 아니라 규칙이 잘못 재고 있었습니다.** 이로써 `fcb9490`의 예산 가설은 이 건의 원인이 아님이 확인됐습니다(그 변경 자체는 옳아 유지).
- Change and reason:
  - `ANSWER_TOO_SHORT`가 견주는 기준을 원문 길이에서 **`min(목표 글자 수, 원문 길이)`**로 바꿨습니다. 위쪽 한도(`LENGTH_OVER`)는 이미 `max(목표, 원문)`으로 목표를 함께 보고 있었는데, 아래쪽만 보지 않고 있었습니다.
  - 원문만 보면 정상적인 두 경우가 막힙니다. **첫째,** 지원자가 원문보다 짧은 목표를 지정한 경우 — 2,000자를 700자로 줄이는 것은 시킨 일인데 35%라 걸립니다. **둘째,** CREATE·BUILD에서 원문 자리에 오는 것은 완성된 글이 아니라 사실 메모입니다(프롬프트가 그렇게 설명합니다). 메모를 길게 적을수록 기준이 올라가, 잘 쓴 700자 답변이 메모 2,500자의 28%라며 걸립니다. 실패 건이 FINAL·**내용 보완(BUILD)**이었던 것이 이와 맞습니다.
  - 목표가 원문보다 길면(BUILD가 채워 주기로 한 분량) 원문을 기준으로 둡니다 — 아직 쓰지 않은 분량을 근거로 실패시킬 수는 없습니다.
  - 껍데기를 막는 원래 목적은 유지됩니다: 700자를 부탁했는데 100자가 오면 여전히 걸립니다. 실제 사고였던 "1,687자가 65자로" 판도 그대로 걸립니다(테스트로 고정).
  - **완화되는 구간이 하나 있습니다.** 목표 700자에 362자가 오는 식(목표의 52%)은 이제 통과합니다. 예전에는 원문이 길면 걸렸습니다. 유료 건을 환불시키는 쪽보다 얇더라도 전달하는 쪽을 택했습니다 — `BLOCKING_VALIDATION_CODES` 주석의 판단 기준과 같습니다. 더 조이려면 목표 대비 절대 하한을 따로 두는 편이 낫습니다(후속 과제).
...
### 2026-09-06 KST — 간편 입력에 "더 정확하게 써드릴게요" 두 칸(사실 재료 / 방향 지시)

- Agent/session: Claude (github-gui-sync-jfbyd5), 사용자 요청. 배경: 같은 자소서를 ChatGPT와 비교했더니 ChatGPT가 이겼는데, 이유가 모델 실력이 아니라 **재료 차이**였습니다 — 무아레쥬메·헬띠루틴·Transtream처럼 제출 서류에 없던 사실을 ChatGPT는 대화로 알고 있었고, 여기는 알 방법이 없었습니다.
- Status: active. 마이그레이션 없음.
- Protected baseline: 상세 입력의 기존 칸(`AdditionalInfoInput`, 구조화된 추가 경험, 자격·스펙)은 그대로입니다. 상태를 나눠 쓰지 않고 간편 입력 전용 상태를 새로 뒀습니다 — 공유하면 한쪽에서 지운 것이 다른 쪽에서 사라진 것처럼 보입니다.
- 확인된 사실: 두 통로가 **이미 데이터 모델에 있었습니다**(`candidateMaterials.freeformNotes`, `revisionRequest`). 간편 입력에서만 칸이 없었고, 저장할 때 `inputMode === "SIMPLE" ? "" : freeformNotes`로 **비워서 보내고** 있었습니다.
- Change and reason:
  - **빈 칸 하나를 만들지 않았습니다.** "AI에게 하고 싶은 말" 빈 칸은 작은 챗봇이고, 그러면 "그냥 챗지피티 쓰지"가 맞는 말이 됩니다. 빈 칸은 **무엇을 말해야 하는지 이미 아는 사람**에게만 쓸모가 있습니다 — 긴 대화 끝에 스스로 알아낸 사람은 잘 쓰고, 처음 온 사람은 아무것도 못 씁니다. 우리는 자소서와 공고를 이미 읽었으므로 무엇을 물어야 하는지 압니다. 그래서 **묻는 칸**으로 만들었습니다: 단추를 누르면 예시가 아니라 채워 넣을 **틀**이 들어갑니다(`수료·교육`, `자격증`, `만든 것·운영한 것`, `수상·수치`, `공백기` / `이걸 강조`, `이건 빼줘`, `담백하게`, `적극적으로`).
  - **두 칸으로 나눈 이유.** 위 칸은 사실이고 아래 칸은 지시입니다. 검증기는 근거로 인용된 문구가 지원자 자료에 실제로 있는지 보는데, `revision_request`는 근거 원천에서 빠져 있습니다. "에이텍 빼주세요"가 근거로 인용되면 그것이 바로 이 검사가 막으려는 지어낸 근거이고, "정보처리기사 2024"는 반대로 근거로 쓰여야 합니다. 한 칸에 섞으면 둘 중 하나는 반드시 잘못 처리됩니다. 갈라져서 도착하는지를 테스트로 고정했습니다.
  - 프롬프트는 이미 필요한 규칙을 다 갖고 있었습니다 — 요청사항을 인용하지 말 것, 빼달라면 실제로 뺄 것, **없는 경험을 넣어달라는 요청은 따르지 말고 `consultingAdvice`에 이유를 적을 것**. 문구 하나만 고쳤습니다: "이전 첨삭 결과를 보고 다시 요청했습니다" → 이제 첫 분석에서도 요청이 들어오므로, 있지도 않은 이전 결과를 모델이 전제하지 않도록 중립으로 바꿨습니다.
  - 이어받은 요청(`carriedRequest`)과 이번에 적은 요청을 합쳐 보냅니다. 간편 입력은 이어받은 요청을 화면에 보여 주지 않으므로, 둘 중 하나만 고르면 손님이 본 적도 없는 문장 때문에 방금 적은 것이 사라집니다.
  - UI: 기본 접힘(선택 항목이 펼쳐져 있으면 필수처럼 보이고, 간편 입력을 고른 사람은 칸이 적어서 고른 것입니다). PC는 두 칸 좌우 배치, 모바일은 세로. 칩은 **가로 스크롤** — 좁은 화면에서 줄바꿈시키면 단추만 세 줄을 차지합니다.
- Files: `src/components/intake-notes.tsx`(신규), `intake-notes.module.css`(신규), `src/components/pro-input-page.tsx`, `src/server/ai/quick/prompt.ts`, `src/application/application-case-handoff.test.ts`.
...
### 2026-09-06 KST — 문항별 글자 수가 눌러도 안 바뀌던 문제, 고칠 수 있는 칸으로 안 보이던 문제, 첨부 자소서가 조용히 밀려나던 문제

- Agent/session: Claude (github-gui-sync-jfbyd5), 사용자 제보 세 건.
- Status: active. 마이그레이션 없음.
- Change and reason:
  - **눌러도 숫자가 안 바뀌던 것.** 고친 값은 본문 제목 뒤 `(700자)` 표시로 저장되고, 그 표시가 읽히는 범위는 100~9999입니다. 글자를 칠 때마다 바로 쓰고 있었으므로 "3"과 "30"을 지나가는 순간마다 범위 밖이라 표시가 지워졌고, 지워지면 값이 전체 기본값으로 되돌아왔습니다 — **치는 족족 되돌려 놓고 있었습니다.** 앞서 검증할 때는 값을 통째로 한 번에 넣어서(`"300"`) 이 경로를 지나가지 않았고, 그래서 통과한 것으로 보였습니다.
  - 치는 동안에는 화면에만 담아 두고(`typing`), **범위에 들어오는 순간 바로 반영**합니다. 칸을 벗어나야만 저장하면, 숫자를 고치고 곧장 다음 단추를 누른 사람의 값이 사라질 수 있습니다. 100 미만으로 두고 나가면 표시를 지워 전체 기본값으로 되돌립니다.
  - **고칠 수 있는 칸으로 안 보이던 것.** 테두리를 투명으로 두고 마우스를 올렸을 때만 보이게 했더니, 여기가 눌러서 바꾸는 자리인 줄 아무도 몰랐습니다. 쉬는 상태에서도 테두리와 흰 배경을 둡니다.
  - **첨부한 자기소개서가 조용히 밀려나던 것.** `mapSimpleIntake`가 "친 글이 이긴다"로 정해 두었습니다 — 새 원고를 붙여넣고 예전 사본이 함께 있을 때는 맞는 규칙입니다. 그런데 이 칸에 참고사항 한 줄을 적는 순간에도 같은 일이 벌어져 **첨부한 자기소개서가 통째로 분석에서 빠집니다.** 아무 표시가 없어 손님은 자기 자소서가 빠진 줄도 몰랐습니다.
  - 규칙 자체는 바꾸지 않았습니다. 친 글이 자소서인지 메모인지 기계가 알아맞히게 하면, 틀렸을 때 **엉뚱한 자소서를 분석해 돈을 받는** 결과가 됩니다 — 조용히 빠지는 것보다 나쁩니다. 대신 무슨 일이 벌어지는지 화면에 말하고, 참고사항은 새로 생긴 "더 정확하게 써드릴게요"에 적으면 자소서를 밀어내지 않는다고 안내합니다. 이 동작을 테스트로 못박아, 규칙을 바꾸면 안내도 같이 손보게 했습니다.
- Files: `src/components/simple-intake.tsx`, `simple-intake.module.css`, `src/domain/simple-intake-mapping.test.ts`.
- Validation: `npx tsc --noEmit` clean, `npx vitest run` 1007 passed(신규 1건), `npx eslint src` 오류 0건. 로컬 dev에서 한 글자씩 치는 경로를 그대로 흉내내 확인했습니다 — "3"·"30"에서는 본문이 그대로고 "300"에서 `2. 직무 역량 (300자)`가 붙으며, 다른 문항은 700을 유지합니다. 첨부 자소서 안내는 파일 업로드가 필요해 브라우저로 못 보고 매핑 동작만 테스트로 확인했습니다.
- Rollback: 이 커밋 revert.
- 남은 판단: 사용자는 "자소서를 파일로 냈으면 이 칸을 추가정보 입력란으로 써도 되지 않느냐"고 물었습니다. 맞는 방향이지만, 친 글이 자소서인지 메모인지 가르는 기준이 필요합니다. 지금은 안내로 갈라 두었고, 규칙을 바꾸려면 그 기준부터 정해야 합니다.

### 2026-09-06 KST — 자소서 밖으로 넓히기 위한 구도 문서 (설계만, 코드 없음)

- Agent/session: Claude (desktop), 사용자 요청("이력서·경력기술서·포트폴리오·대학원 진학서·법률 서면까지 런칭 준비, 커리어 검사처럼 왼쪽 접힌 드로어에 마크 하나 더, 열면 목록에서 골라 프로 빌드 심플 유형으로 진행. 일단 구도부터").
- Status: **설계안**. 소스·스키마·프롬프트 변경 없음. 마이그레이션 없음.
- Change and reason: `docs/multi-document-expansion-plan.md` 신규. 조사해 보니 자소서 전제가 가장 깊은 곳은 프롬프트(`prompt.ts:127` 페르소나 줄과 문항·면접관 규칙)이고, 요청 계약과 문서 분류는 얕았습니다. 그리고 이 제품의 코어는 "자소서를 잘 쓴다"가 아니라 **근거 검증**(`evidenceQuote`가 제출 문서에 실제로 있는 문구여야 한다는 규칙)이라, 문서 종류를 바꿔도 그대로 값을 합니다. 그래서 확장을 "새 제품"이 아니라 **같은 엔진에 입출력 껍데기 갈아 끼우기**로 잡았습니다.
  - 축을 하나 더합니다: `product × writingMode`에 `docType`. 세 축을 곱하지 않고 종류별로 의미 있는 조합만 엽니다(이력서에 CREATE는 열지 않음 — 없는 경력을 쓰면 위조).
  - "문항"을 "구획"으로 일반화하되 **필드 이름(`questions[]`, `questionOrder`)은 바꾸지 않습니다.** 이름을 바꾸면 저장된 과거 결과와 렌더러까지 따라 움직이는데 얻는 것은 용어의 정확함뿐입니다. 화면 라벨만 종류별로 다르게 합니다.
  - 드로어는 `CareerAssessmentDrawer`를 고치지 않고 **형제 컴포넌트**로 새로 만듭니다. CSS도 복사해서 시작합니다 — 공유하면 커리어 쪽 조정이 서류 쪽을 깨뜨립니다.
  - 라우트도 `/final/*`을 건드리지 않고 `/write/[docType]/[mode]` 형제 계열로 둡니다(색인된 주소이고 지금 매출이 나오는 경로).
  - **법률 서면은 범위를 좁혀야 열 수 있다고 적었습니다.** 이 영역에서 모델이 가장 잘 저지르는 실수는 문장이 아니라 법조문·판례 번호를 지어내는 것이고, 기한(항소기간)을 잘못 안내하면 회복 불가능한 피해가 됩니다. 그래서 "구조·연결·가독성만 손대고, 인용·기한 안내·사실 보충·CREATE 모드는 열지 않는다"로 선을 그었습니다. 인용 차단은 지금 근거 검증기와 같은 방식으로 재사용 가능합니다. 첫 상품으로는 위험이 낮고 수요가 비슷한 내용증명·진정서를 권했습니다.
  - 런칭 3차수와 종류별 `enabled` 환경변수 게이트(커뮤니티 자동글과 같은 방식)를 함께 적었습니다.
- Files: `docs/multi-document-expansion-plan.md`(신규).
- Validation: 문서만 추가했습니다. 조사 근거는 문서 1절 표에 파일·행으로 적어 두었습니다.
- Rollback: 문서 삭제.
- 다음: 문서 8절의 결정 5가지(가격 축, 드로어 라벨, 법률 첫 상품, 1차 범위, 영문 서류)에 답이 나오면 1차(이력서·경력기술서) 구현에 들어갑니다.

### 2026-09-06 KST — 자소서를 파일로 낸 사람은 문항별 글자 수를 고쳐도 그대로 사라지던 문제

- Agent/session: Claude (github-gui-sync-jfbyd5), 사용자 제보("300하고 떼면 바로 700 돌아간다").
- Status: active. 마이그레이션 없음.
- 원인: 고친 값을 **본문 제목 뒤의 `(700자)` 표시로** 저장하고 있었습니다. 그런데 사용자 화면은 자기소개서를 **파일로** 낸 상태였고(입력창은 비어 있고 첨부 1개), 그러면 `writeQuestionTargetLength`가 손댈 본문이 없어 원본 문자열을 그대로 돌려줍니다. 저장이 일어나지 않으니 다시 그릴 때 전체 기본값(700)으로 돌아갔습니다. **붙여넣은 사람만 되고 첨부한 사람은 안 되는 기능이었습니다** — 앞선 검증은 붙여넣은 경우만 봐서 통과한 것으로 보였습니다.
- Change and reason:
  - 표시에 의존하지 않고 **화면 상태로 따로 받습니다**. `mapSimpleIntake(draft, files, defaultTargetLength, targetOverrides)`에 문항 순번을 열쇠로 한 값을 넘기고, 그 값이 있으면 그 문항의 목표 글자 수로 씁니다. 본문에서 오든 파일에서 오든 문항 목록은 같은 방식으로 만들어지므로, 한쪽만 되던 문제가 사라집니다.
  - 지원자가 본문에 직접 적은 `(800자)` 표시는 **그대로 읽습니다**(고용주가 문항 옆에 인쇄해 둔 숫자입니다). 다만 화면에서 손으로 고친 값이 있으면 그쪽이 이깁니다 — 나중에 한 행동이 앞의 것을 덮는 것이 사람이 기대하는 순서입니다.
  - **본문을 더 이상 건드리지 않습니다.** 숫자를 고칠 때마다 손님이 붙여넣은 글에 `(300자)`가 끼어들던 것도 함께 없어졌습니다.
  - 쓰이지 않게 된 `writeQuestionTargetLength`와 그 테스트를 지웠습니다. 같은 일을 하는 길이 둘이면 다음 사람이 어느 쪽이 진짜인지 알 수 없습니다.
- Files: `src/domain/simple-intake-mapping.ts`, `simple-intake-mapping.test.ts`, `src/domain/cover-letter-parser.ts`, `cover-letter-parser.test.ts`, `src/components/simple-intake.tsx`, `simple-intake.test.tsx`, `src/components/pro-input-page.tsx`.
- Validation: `npx tsc --noEmit` clean, `npx vitest run` 1005 passed(신규 3건 · 삭제 5건), `npx eslint src` 오류 0건. 로컬 dev에서 한 글자씩 치는 경로 확인 — "3"·"30"에서는 저장하지 않고 "300"에서 반영되며, 칸을 벗어나도 300이 유지되고 2번 문항 감소율이 -25%로 다시 계산됩니다. **본문은 그대로입니다.** 파일로 낸 경우는 업로드가 필요해 브라우저로 못 보고 매핑 테스트로 확인했습니다.
- Rollback: 이 커밋 revert. 되돌리면 파일로 낸 사람은 다시 고칠 수 없게 됩니다.

## 2026-09-06 — Claude: 개인정보처리방침을 법 항목별로 3분할 (LINER식 구조)

- 사용자 지적: 회사 이름을 한 표에 몰아 적으니 개인정보 고지가 아니라 **기술 스택 목록**으로 읽힙니다. LINER 사례를 확인해 보니 그쪽은 공개를 줄인 것이 아니라 **법 항목별로 흩어 놓아서** 설계도로 읽히지 않는 구조였습니다.
- 앞 커밋(`322ff6f`)에서 업무 설명만 일반화했는데, 그것만으로는 한 표에 몰린 문제가 남았습니다. 이번에 구조를 나눴습니다.
  - **4. 개인정보 처리업무의 위탁** — 맡긴 *업무*만 목록으로. 회사 이름 없음. 재위탁 제한·목적 외 처리 금지 등 관리·감독 문구 추가.
  - **5. 개인정보의 국외 이전**(신설) — 수탁자가 전부 국외 사업자이므로 여기에 법정 기재사항과 함께 적습니다. 이전받는 자 / 국가 / 이전 항목 / 이용 목적 / 보유·이용 기간을 업체별로, 그리고 이전 시기·방법, 거부 방법과 효과, 연락처 안내를 별도 문단으로 두었습니다.
  - **8. 자동 수집 장치 및 행태정보** — Google(광고 성과 측정), Microsoft Clarity(이용 흐름 분석)를 수집 항목·목적·거부 방법과 함께. 지원 자료가 이 도구들에 전달되지 않는다는 점을 명시했습니다.
- **공개량은 오히려 늘었습니다.** 국외 이전은 항목·목적·보유기간까지 요구하므로 이전보다 더 많이 적습니다. 줄인 것은 "어떤 저장소에 무엇이 들어가고 인증을 무엇으로 하는지"뿐입니다.
- 절 번호를 5~11 → 6~12로 밀고 상호참조 3곳(2번의 "아래 6번", 국외이전의 보호책임자 참조, 9번의 "선택 동의(6번)")을 새 번호로 맞췄습니다.
- **하지 않은 것(사용자 요청 중 반려, 이유 설명함)**: 글자 크기 축소·문서 하단 이동. 법이 "정보주체가 언제든지 쉽게 확인할 수 있도록" 공개하라고 요구하므로, 읽기 어렵게 만드는 것은 항목별로 나누는 것과 성격이 다릅니다.
- **OpenAI는 뺄 수 없습니다**(앞 커밋에서 확인). `redactPersonalData`가 `research-capture.ts`에서만 쓰이고 분석 경로에는 적용되지 않아, 자기소개서가 원문 그대로 전달됩니다. 분석 경로에도 마스킹을 넣으면 재검토 가능 — 다만 첨삭 품질에 영향이 있어 사용자 확인 후 진행할 사안으로 남겨 두었습니다.
...
### 2026-09-06 KST — 좁은 화면에서 간편 입력 카드가 옆 카드들보다 튀어나오던 문제

- Agent/session: Claude (github-gui-sync-jfbyd5), 사용자 제보(406px 화면 스크린샷).
- Status: active. 마이그레이션 없음.
- 원인: `.intake`는 그리드인데, **그리드 항목은 기본값이 `min-width: auto`라 내용의 최소 폭보다 좁아지지 않습니다.** 좁은 화면에서 칸은 232px인데 카드 두 장(`.box`, `.sorted`)이 290px로 버티며 밖으로 밀려났고, 그래서 위아래 다른 카드들과 오른쪽 끝이 어긋나 보였습니다. 320px에서는 화면 자체가 11px 넘쳤습니다.
- 처음에 잘못 짚은 것: 긴 파일 이름(`대학일자리센터_직업상담사_..._전민수.pdf`)이 낱말 단위로 안 쪼개져서라고 보고 줄바꿈 규칙부터 손댔습니다. 실제로 재어 보니 그 이름이 든 `<b>`는 177px뿐이었고, 규칙을 되돌려 측정해도 결과가 같았습니다 — 원인이 아니었습니다. 낱말 쪼개기 규칙은 그 자체로 옳아 남겨 두되, 이 문제의 해결책은 아닙니다.
- Change and reason:
  - `.intake > * { min-width: 0 }`. 이 한 줄로 카드들이 칸 안으로 들어옵니다.
  - 확인: 320px에서 화면 넘침 11px → **0**, 406px에서 카드 네 장의 좌우 끝이 모두 `41→365`로 정렬됩니다(전에는 간편 입력 카드만 더 넓었습니다).
- Files: `src/components/simple-intake.module.css`.
...
### 2026-09-06 KST — 유료 칸을 반짝이·알약 라벨 없이 다시 그린다 (블루·화이트)

- Agent/session: Claude (desktop), 사용자 지시("이런 마크는 이제 사용금지 — AI로제작 앞 마크 제거/변경, 'AI RESUME BUILD · 유료' 같은 카드 부제목 금지, 블루·화이트 계열로 글자 작고 오밀조밀하게, 토스처럼 입체감 있고 투박하지 않게, UI가 더 부각되게" + 스크린샷).
- Status: active. 바로 앞 커밋(`ae7cd5c`)의 겉모습만 고칩니다 — 결제·API·도메인 로직은 한 줄도 건드리지 않았습니다.
- Change and reason:
  - **반짝이(Sparkles) 아이콘을 뺐습니다.** 상단바 "AI로 제작"은 아이콘 없이 글자만 둡니다. 좁은 화면에서 다른 단추는 글자를 감추고 아이콘만 남기는데 이 단추에는 아이콘이 없으므로, 대신 **짧은 글자("AI")로 바뀝니다**(`.buttonShort`) — 아이콘만 남기면 무엇을 하는 단추인지 알 수 없고, 글자를 그대로 두면 바가 두 줄로 터집니다. 결제 단추의 반짝이는 화살표로, 편집 화면 "기술 · 역량" 제목의 반짝이는 렌치로 바꿨습니다.
  - **알약 라벨("AI RESUME BUILD · 유료")을 걷어냈습니다.** 광고 배너처럼 읽혀서 정작 이 칸이 무엇을 하는지가 뒤로 밀렸습니다. 값은 라벨이 아니라 값이므로 **오른쪽 위 네모 칩**으로 옮겼습니다 — 얼마인지 보려고 아래까지 내려가지 않아도 됩니다.
  - **짙은 초록 바탕 → 옅은 파랑 바닥 + 흰 카드.** 위쪽 무료 도구가 초록·흰 계열이라 색만으로 "여기서부터 다른 것"이 읽히고, 카드를 그림자로 띄우면 라벨 없이도 눈이 먼저 갑니다. 그림자는 두 겹입니다(가까운 것은 테두리를 또렷하게, 먼 것은 카드를 띄웁니다) — 하나만 쓰면 번지거나 딱딱해집니다.
  - 글자를 한 단계씩 줄이고(제목 21→18px, 본문 12.5→12px, 보조 11→10.5px) 여백을 좁혔습니다. 입력 칸이 많은 화면이라 여백이 크면 한 화면에 안 들어옵니다. 누를 때 1px 내려앉는 것 외에 애니메이션은 넣지 않았습니다 — 더 주면 장난감처럼 보입니다.
...
## 2026-09-06 — Claude: 법률 값을 정하고, 1,400쪽이 안 읽힌다는 사실을 화면이 말하게 한다

- Agent/session: Claude, 사용자 결정(법률문서 99,000 통일 · 사건자료분석 별도 · 법률 홈 단계 구성).
- Status: active(같은 브랜치, 여전히 `preview` — 목록에 안 나오고 색인도 막혀 있음).

- **값**(사용자 결정): 법률 문서 99,000원 통일, 사건자료 분석만 별도 149,000원(사용자가 제시한 99,000~199,000의 가운데). `legalDocumentPriceKrw(docType)`가 한 곳에서 정하고, 결제 건에 적힌 문서 종류로 다시 부르므로 싼 문서를 사서 비싼 문서를 만드는 길이 없습니다. 취업 쪽(경력기술서·포트폴리오)은 손대지 않았습니다 — 99,000 통일이 그쪽까지인지 확인이 필요합니다.
- **1,400쪽 문제 — 지금 구조는 못 읽습니다.** 상한이 6만 자(약 40쪽)라, 의료기록 1,400쪽을 넣으면 앞 3%만 읽습니다. 그런데 **그 사실이 화면에 하나도 안 나왔습니다.** 99,000원짜리 상품에서 이건 기능의 한계가 아니라 사고라, 세 곳을 고쳤습니다.
  1. 결제 단추 위에 경고: "약 1,400쪽인데 앞 40쪽까지만 읽습니다. 나머지 N자는 반영되지 않습니다."
  2. 결과와 함께 다 읽지 못한 자료 이름을 보여 줍니다(`truncated`를 화면이 그냥 버리고 있었습니다).
  3. 파일 하나가 저장 상한을 넘으면 앞부분만 저장하고 몇 쪽 중 몇 쪽인지 말합니다. 전에는 서버가 형식 오류(400)로 막았는데, 큰 파일을 올린 사람에게 "형식이 올바르지 않습니다"는 파일을 고치라는 말로 읽힙니다.
  - 근본 해결(청킹·OCR·RAG)은 별도 작업입니다. 아래 "다음"에 적습니다.
- **법률 홈 단계 재구성**(사용자 지정): `사건부터 이해하기 / 소송 전 / 1심 / 판결 후`. 문서 이름이 아니라 **사람이 서 있는 자리**로 묶습니다 — 소장을 받은 사람은 "답변서"라는 낱말은 몰라도 자기가 1심에 있다는 것은 압니다.
- **사건 현황 줄**: 자료 쪽수·자료 개수·주요쟁점·증거·작성 문서. **가진 값만 셉니다** — 사건 분석을 만들기 전에는 쟁점·증거 칸이 아예 없습니다. 화면이 "쟁점 8개"라고 말하려면 그 8개를 눌러 볼 수 있어야 합니다(타임라인은 아직 없어서 넣지 않았습니다).
...
## 2026-09-11 — Claude: Google Play Billing(TWA)을 두 번째 결제 진입점으로 추가하기로 하고, 새 브랜치에서 착수 (착수 기록, 작업 전)

- Agent/session: Claude, 사용자 요청("웹앱인데 구글빌링연결하는걸로하자"). Plan mode로 조사 후 사용자 승인.
- Status: active, 착수. 브랜치 `feature/google-play-billing`(HEAD 기준, `origin/main` 포함 확인 후 분기 — `main`에서 갈라지지 않고 이 세션이 있던 `claude/job-description-input-attach-cx44pf`(이미 main을 포함한 상태) 위에서 분기).
- Protected baseline: 기존 Polar 결제 경로 전체 — `src/server/billing/polar-checkout.ts`, `polar-webhook.ts`, `supabase-polar-entitlement-repository.ts`, `checkout-route.ts`, `quick-checkout-service.ts`, 관련 RPC(`prepare_quick_checkout`/`register_quick_checkout`/`grant_polar_order_entitlement`/`mark_polar_checkout_succeeded`/`refund_polar_order_entitlement`/`settle_referral_for_order`)와 `billing_orders`/`checkout_intents`/`billing_webhook_events`의 기존 `'POLAR'`/`'MOOA_CREDIT'` 행. 어느 것도 수정·삭제하지 않고 병행 추가만 한다.
- Change and reason(계획): 일반 웹 브라우저는 기존 Polar 결제를 그대로 쓰고, Play Store에서 설치한 TWA 앱 안에서만 Google Play Billing으로 분기한다(Play 정책상 앱 내 디지털 상품 결제는 Play Billing 의무 사용). 사이트를 새로 만들지 않고 같은 Next.js 웹앱에 결제 진입점 하나를 추가하는 구조. 상세 계획은 `C:\Users\jeonm\.claude\plans\crispy-yawning-avalanche.md`(Phase A: 이 저장소 코드 — PWA 매니페스트 보강, `public/.well-known/assetlinks.json`, `src/server/billing/google-play-checkout.ts` 신규, DB 마이그레이션 신규 파일(`billing_orders`/`billing_webhook_events` provider 체크에 `'GOOGLE_PLAY'` 추가, `grant_google_play_order_entitlement` 신규 함수 — `grant_polar_order_entitlement`를 고치지 않고 `consume_reward_credit` 전례를 따라 병행 추가, `settle_referral_for_order`의 `provider <> 'POLAR'` 하드게이트 완화), `SupabaseGooglePlayEntitlementRepository` 신규, `src/app/api/billing/google-play/verify/route.ts` 신규, `src/components/application-case-handoff.tsx`의 결제 버튼에 `getDigitalGoodsService` 존재 여부 분기 추가; Phase B: Play Console/서명 키/인앱상품/서비스 계정은 사용자가 직접, 별도 저장소의 Bubblewrap TWA 프로젝트도 사용자 몫; Phase C: 실구매 1건으로 연결 검증).
- Files/branch: 아직 파일 변경 없음(이 항목이 착수 기록). 이후 커밋에서 구체적 파일 목록을 추가한다.
- Validation: 착수 시점, 해당 없음. Phase A 완료 시 `npx tsc --noEmit`, lint, vitest로 검증 예정.
- Rollback/recovery reference: 새 브랜치 `feature/google-play-billing`이므로 `main`/기존 세션 브랜치에는 영향 없음. DB 마이그레이션은 모두 신규 파일 추가(기존 함수 `CREATE OR REPLACE` 없음, `grant_polar_order_entitlement` 등 Polar 전용 함수는 무수정)라 되돌릴 때 새 마이그레이션 파일만 없던 일로 하면 된다.
- User decision: 사용자가 "웹=Polar / Play 앱=Google Billing" 구조와 "TWA 안드로이드 프로젝트는 별도 저장소" 둘 다 명시적으로 선택(AskUserQuestion). Phase A만 지금 진행, Phase B/C는 사용자가 Play Console 준비되면 이어서.

## 2026-09-11 — Claude: Google Play Billing Phase A 코드 완료 (Polar 미수정, 마이그레이션은 아직 원격에 push 안 함)

- Agent/session: Claude, 같은 세션, 바로 위 착수 기록의 계획을 그대로 구현.
- Status: active, `feature/google-play-billing` 브랜치에 로컬로 존재. **커밋하지 않음** — 사용자가 커밋을 명시적으로 요청하지 않아 요청 전까지 작업트리에만 둠. **DB 마이그레이션도 아직 원격 Supabase에 push하지 않음** — 이 환경에 `SUPABASE_ACCESS_TOKEN`이 없어 `supabase db push --dry-run`조차 못 돌렸고, 결제 스키마를 건드리는 이전 사례(2026-09-07 기록)와 같은 원칙대로 push는 사용자 승인 후에 한다.
- Protected baseline: 착수 기록과 동일. `git diff`로 Polar 관련 파일(`polar-checkout.ts`, `polar-webhook.ts`, `supabase-polar-entitlement-repository.ts`, `checkout-route.ts`, `quick-checkout-service.ts`, 기존 마이그레이션 전부)에 변경이 없는지 확인함 — 전부 무수정, 새 파일 추가와 `application-case-handoff.tsx`/`manifest.ts` 두 파일의 국소 수정만 있음.
- Change and reason: 계획대로 Phase A를 구현.
  - **신규**: `supabase/migrations/20260911010000_google_play_billing.sql`(`billing_orders`/`billing_webhook_events`의 provider 체크에 `'GOOGLE_PLAY'` 추가, `grant_google_play_order_entitlement` 함수 신규 — `consume_reward_credit` 전례를 따라 `grant_polar_order_entitlement`는 무수정, `settle_referral_for_order`를 `provider not in ('POLAR','GOOGLE_PLAY')`로 재정의), `src/server/billing/google-play-checkout.ts`(env 설정/서비스계정 JSON 파싱/실패 분류, `polar-checkout.ts`와 같은 3함수 패턴), `src/server/billing/google-play-verification.ts`(구매 검증+엔타이틀먼트 지급 오케스트레이션, `polar-webhook.ts`의 `processPolarWebhook`에 대응), `src/server/billing/supabase-google-play-entitlement-repository.ts`(Polar 리포지토리와 같은 모양, 의도적으로 코드 중복 — 아래 이유 참고), `src/server/billing/google-play-verify-route.ts` + `src/app/api/billing/google-play/verify/route.ts`(얇은 라우트, `checkout-route.ts` 패턴), `src/lib/google-play/purchase.ts`(클라이언트: Digital Goods API + Payment Request API로 구매 완료 후 토큰 반환), `public/.well-known/assetlinks.json`(placeholder, `package_name`/`sha256_cert_fingerprints`는 Phase B에서 사용자가 채움 — 빈 지문 배열이라 지금은 아무 것도 검증하지 않음).
  - **수정(국소)**: `src/app/manifest.ts`(`id`/`scope` 추가, 아이콘에 `purpose: "maskable"` 항목 병행 추가 — Next.js 매니페스트 타입이 스펙의 공백구분 다중값을 안 받아 항목을 나눔), `src/components/application-case-handoff.tsx`의 `beginCheckout` 함수 맨 앞에 `isGooglePlayBillingAvailable()` 분기만 추가 — 기존 Polar `fetch` 이하 로직은 그 블록 밖에서 한 줄도 건드리지 않음. Play 분기는 구매 완료 → 서버 검증(`/api/billing/google-play/verify`) → 결과에 따라 `/api/analysis-runs/quick/execute` 직접 호출 → `onCreditRunStarted` 콜백(기존 무료 이용권 경로 `startWithCredit`가 쓰는 것과 동일한 콜백을 재사용) 순서로, Polar처럼 체크아웃 URL로 리다이렉트했다가 돌아오는 구조가 아니라 이 자리에서 바로 끝난다.
  - **패키지**: `googleapis`를 `dependencies`에 추가(`npm install googleapis`), Play Developer API(`androidpublisher` v3)용.
  - **의도적으로 안 한 것 (계획에서 벗어난 부분)**: 계획서 A9에 적었던 `scripts/check-e2e-readiness.mjs`의 `REQUIRED_ENV_GROUPS`에 `google_play` 그룹 추가를 하지 않았습니다. 이유: 그 스크립트의 `ready` 판정은 모든 그룹이 갖춰져야 `true`가 되는데, Google Play는 웹 사이트가 정상 동작하는 데 필수가 아니라 Play 앱 안에서만 쓰이는 선택 기능입니다(웹은 여전히 Polar만으로 완결). 그룹을 추가하면 이 선택 기능이 없다고 e2e readiness 전체가 실패로 뜨고, 기존 보호 대상 테스트 `src/server/config/e2e-readiness.test.ts`의 "accepts a complete sandbox configuration"(Polar만 채운 값으로 `ready === true`를 기대)이 깨집니다. 계획을 쓸 때는 이 테스트를 미처 다시 확인하지 않았습니다 — 실제 구현 중 발견하고 스크립트는 그대로 두는 쪽으로 바꿨습니다.
  - **가격 관련 알려진 제약 (문서화, 코드로 처리)**: Play Console 인앱상품은 고정가 하나뿐이라 Polar처럼 분량에 따라 "추가 블록"을 계산해 얹는 방식이 없습니다. `google-play-verification.ts`는 `quote.extraBlocks > 0`이면(QUICK 8,000자·PRO/FINAL 30,000자 초과) Play 결제 자체를 거절하고 "웹 브라우저에서 결제해 주세요"라고 안내합니다 — 부족한 이용권을 조용히 지급하는 대신 거절을 택했습니다. 또한 Play Developer API의 구매 조회 응답에는 실제 결제 금액이 없어(카탈로그 가격만 Play Console에 있음), 기록되는 `amount`는 실제 청구액을 되읽은 값이 아니라 이 코드가 기대하는 카탈로그 기준가입니다 — 코드 주석에 남겨 둠.
- Files/branch: 신규 — `supabase/migrations/20260911010000_google_play_billing.sql`, `src/server/billing/google-play-checkout.ts`(+`.test.ts`), `src/server/billing/google-play-verification.ts`(+`.test.ts`), `src/server/billing/supabase-google-play-entitlement-repository.ts`, `src/server/billing/google-play-verify-route.ts`, `src/app/api/billing/google-play/verify/route.ts`, `src/lib/google-play/purchase.ts`, `public/.well-known/assetlinks.json`. 수정 — `src/app/manifest.ts`, `src/components/application-case-handoff.tsx`, `package.json`/`package-lock.json`(googleapis 추가). 브랜치 `feature/google-play-billing`.
- Validation: `npx tsc --noEmit -p .` clean. `npx eslint`(변경/신규 파일 전체) 오류 0건. `npx vitest run` 140 files · 1116 tests 전부 통과(신규 14건 포함, 기존 테스트 전부 그대로 통과 — 회귀 없음). `npm run build`(Cloudflare용이 아닌 순수 `next build`) 성공, `/api/billing/google-play/verify`와 `/manifest.webmanifest` 라우트가 정상적으로 목록에 나옴. `supabase db push --dry-run`은 이 환경에 로그인 토큰이 없어 못 돌림 — 사용자가 로컬/CI에서 한 번 더 돌려보는 것을 권장.
- Rollback/recovery reference: 전부 새 브랜치의 신규 파일 또는 위 두 파일의 국소 추가라, 되돌릴 때는 나열된 신규 파일들을 지우고 `manifest.ts`/`application-case-handoff.tsx`의 추가된 블록만 제거하면 됩니다. 마이그레이션은 원격에 아직 push하지 않았으므로 파일만 지우면 DB에는 아무 흔적도 없습니다.
- User decision: 사용자가 계획(파일 경로 포함)을 승인(ExitPlanMode)한 뒤 구현 지시. 커밋/push, DB 마이그레이션 push, Phase B(Play Console/서명 키/인앱상품 3개/서비스 계정) 진행 여부는 아직 사용자에게 확인받지 않음 — 다음 메시지에서 안내 예정.

## 2026-09-11 — Claude: 가격을 다중 Play 상품 사다리로 바꾸고, Android TWA 프로젝트·서명키·첫 서명 빌드를 별도 저장소에 완료

- Agent/session: Claude, 같은 세션. 사용자 피드백 세 가지에 대응: (1) "db는 명령어 잘못말햇는지 푸시실패함" (2) "4 결제막으면... 가격은 여러개 프로덕트만들면되잖아" (3) "서명 이런건 니가하셈".
- Status: active. 마이그레이션은 **여전히 원격 미반영**(아래 원인 참고). `feature/google-play-billing` 브랜치 로컬에 존재, 커밋 안 함(사용자가 "커밋은 나중에"라고 명시). Android 저장소(`C:\6.mooaresume-android`)는 별도 git 저장소로 커밋 완료(`84a99c4`).

### (1) DB 마이그레이션 push 실패 원인
- 진단(읽기 전용, 아무것도 바꾸지 않음): `npx supabase migration list` → 403 `LegacyDbConfigLoginRoleStatusError`("계정에 권한 없음"). `npx supabase projects list` → 방금 로그인한 Supabase 계정에는 `transtream`(ref `gctvgejeadjaextajqgz`) 프로젝트만 보이고, 이 저장소가 링크된 프로젝트 `oiucnkrknedqyktnwbce`(mooaresume)는 안 보임.
- 결론: 명령어 문제가 아니라 **로그인한 Supabase 계정이 mooaresume 프로젝트에 대한 권한이 없음**(다른 계정으로 로그인했거나, 이 계정이 mooaresume 조직/프로젝트 멤버가 아님). `supabase logout` 후 mooaresume 프로젝트 권한이 있는 계정으로 `supabase login`을 다시 하면 될 것으로 보임 — 사용자 확인 필요.
- 마이그레이션 파일(`20260911010000_google_play_billing.sql`) 자체는 변경 없음, 여전히 push 대기 상태.

### (2) 가격: extraBlocks 있으면 무조건 Play 결제 거절 → 다중 상품 사다리로 교체
- 문제 제기가 맞았음: 이전 구현은 QUICK 8,000자·PRO/FINAL 30,000자를 넘는 모든 분량을 Play 결제에서 거절했는데, 이러면 Play Billing을 붙이는 원래 목적(한국 결제수단 포함, 카드/카카오페이/네이버페이 등 Google이 지원하는 결제수단으로 앱 안에서 완결)이 큰 폭으로 무의미해짐.
- **신규**: `src/domain/google-play-billing.ts` — `MAX_GOOGLE_PLAY_EXTRA_BLOCKS = 3` 상수. Play Console 상품은 고정가 하나뿐이라 분량별로 상품을 미리 여러 개 만들어 두는 사다리 방식(기본 상품 + 추가 블록 1·2·3개용 상품)으로 바꿈 — Polar처럼 결제 시점에 가격을 계산해 보내는 방식이 Play Billing에는 없기 때문.
- **`google-play-checkout.ts` 수정**: `GooglePlayConfiguration.productIds`를 `Record<ProductTier, string>` 단일값에서 `Record<ProductTier, ReadonlyArray<string>>`(사다리, index 0=기본/1~3=추가블록)로 변경. env var는 `GOOGLE_PLAY_{TIER}_PRODUCT_ID`(기존, index 0)에 더해 `GOOGLE_PLAY_{TIER}_EXTRA_{1|2|3}_PRODUCT_ID`를 새로 읽음 — 전부 설정하지 않아도 됨(설정 안 한 자리는 빈 문자열, 그 분량은 여전히 웹으로 안내).
- **`google-play-verification.ts` 수정**: `quote.extraBlocks`를 인덱스로 사다리에서 필요한 상품 id를 찾고, 그 자리가 비어 있을 때만("GOOGLE_PLAY_TIER_UNSUPPORTED") 거절 — 사다리가 채워진 만큼은 정상 결제됨. 지급 금액/글자수도 `quote.basePriceKrw`/`quote.includedCharacters`(옛 코드, 항상 기본값만)에서 `quote.totalPriceKrw`/`quote.allowedCharacters`(추가 블록 포함)로 고침 — 이전엔 애초에 extraBlocks>0이면 전부 막았으니 안 드러났던 버그.
- **`src/lib/google-play/purchase.ts` 수정**: `NEXT_PUBLIC_GOOGLE_PLAY_{TIER}_PRODUCT_ID`/`_EXTRA_{1|2|3}_PRODUCT_ID`를 정적으로 나열(동적 키로는 Next.js가 클라이언트 번들에 인라인하지 못함 — 주석에 이유 남김). `purchaseProductViaGooglePlay(tier, extraBlocks)`로 시그니처 변경.
- **`application-case-handoff.tsx`/`analysis-preparation.tsx` 수정**: `ApplicationCaseHandoff`에 `extraBlocks` prop 추가(부모가 이미 화면 표시용으로 계산해 둔 `quote.extraBlocks` 그대로 전달). 서버가 analysisRunId로 다시 계산한 값으로 최종 검증하므로 이 값 자체는 신뢰 경계 밖(어느 Play 상품을 열지 고르는 UX 용도일 뿐).
- Validation: `npx tsc --noEmit -p .` clean, eslint 0건, `npx vitest run` 140 files·**1117** tests 통과(신규 테스트 재작성 포함, 기존 전부 그대로 통과), `npm run build` 성공.

### (3) Android TWA 프로젝트·서명키·첫 서명 빌드 — 별도 저장소 `C:\6.mooaresume-android`
- 사용자가 AskUserQuestion에서 "별도 저장소(추천)"를 명시적으로 선택. 새 로컬 git 저장소로 생성, 커밋 1개(`84a99c4`) — GitHub에는 안 올림(`gh` 미로그인, 사용자가 원할 때 직접 올리면 됨).
- **서명 키**: `keytool`로 `android.keystore` 생성(RSA 2048, 유효기간 30년, alias `mooaresume`, DN에 실제 사업자 정보 사용 — 상호 gyroscope/울산). 비밀번호는 `crypto.randomBytes`로 생성해 `.keystore-password.txt`(gitignore 처리)에 저장하고 응답에 그대로 출력함 — **사용자가 별도로 백업해야 함, 잃어버리면 Play 앱을 영구히 업데이트 못 함**(README에도 강조).
- **Android SDK/JDK 구성**: 이 컴퓨터엔 이미 JDK 17(Eclipse Temurin)이 있어 재사용. Android SDK는 없어서 command-line tools를 직접 다운로드(`dl.google.com`)·압축 해제하고 `sdkmanager`로 `platform-tools`/`platforms;android-36`/`build-tools;36.1.0` 설치, 라이선스 전부 동의. `~/.bubblewrap/config.json`에 두 경로를 직접 써 넣어 `bubblewrap` CLI의 최초 실행 시 JDK/SDK 설치 여부를 묻는 대화형 프롬프트(비대화형 환경에서 크래시남, `readline was closed`)를 완전히 건너뜀.
- **`twa-manifest.json`은 `bubblewrap init`의 대화형 마법사 대신 직접 작성**(`@bubblewrap/core`의 `TwaManifest.d.ts` 스키마를 읽어서 정확한 필드로 작성). `packageId: com.mooaresume.twa`, `webManifestUrl`은 mooaresume.com의 실제 매니페스트, `features.playBilling.enabled: true`(Play Billing 기능 켜려면 `enableNotifications: true`도 같이 필요 — 안 그러면 `bubblewrap update`가 에러로 거절). `npx @bubblewrap/cli update --skipVersionUpgrade`로 프로젝트 생성(이 명령은 대화형 프롬프트 없이 끝남).
- **빌드 버그 발견 및 우회**: `npx @bubblewrap/cli build`가 이 Windows 환경(Git Bash·PowerShell 둘 다)에서 내부적으로 `gradlew.bat`를 상대경로 `.\` 없이 호출해 "not recognized as an internal or external command"로 실패함(`@bubblewrap/core`의 `GradleWrapper.js`가 `child_process.execFile('gradlew.bat', ..., {shell:true, cwd})`로 호출하는데 cwd가 정확해도 실패 — 재현 확인). 우회: `bubblewrap update`로 프로젝트만 생성하고, 빌드·zipalign·서명은 `.\gradlew.bat`/`apksigner.jar`/`jarsigner`를 직접 호출(정확히 bubblewrap 소스코드가 내부적으로 쓰는 것과 같은 명령/파라미터, `build.js`/`AndroidSdkTools.js`에서 읽어와 그대로 재현).
- 처음 `minSdkVersion: 21`로 시도했다가 `com.google.androidbrowserhelper:billing:1.2.0`(Play Billing 브리지 라이브러리)이 최소 23을 요구해 매니페스트 병합 실패 → `minSdkVersion: 23`으로 고치고 재생성·재빌드해서 해결.
- **결과물**: `app-release-signed.apk`(사이드로드 테스트용), `app-release-bundle.aab`(Play Console 업로드용) — 둘 다 `apksigner verify`/`jarsigner -verify`로 서명 검증 통과 확인. 둘 다 git에는 안 올라감(재생성 가능, `.gitignore`).
- **mooaresume(웹앱) 저장소에도 반영**: `public/.well-known/assetlinks.json`의 placeholder를 실제 `package_name: com.mooaresume.twa`와 이 키의 SHA-256 지문으로 채움. **이 지문은 "업로드 키" 지문입니다** — Play Console에 처음 업로드하면 Google Play App Signing이 별도 "앱 서명 키"로 다시 서명하는 게 기본값이라, 실제 배포판 검증에는 그 키의 지문이 따로 필요합니다(README에 Phase B 항목으로 남김 — 지우지 말고 배열에 추가하면 됨).
- Files: `C:\6.mooaresume-android\` 전체(신규 저장소, 커밋 `84a99c4`), `C:\6.mooaresume\public\.well-known\assetlinks.json`(placeholder → 실값).
- Rollback/recovery reference: Android 저장소는 이번 세션에서 새로 만든 것이라 통째로 지워도 mooaresume 웹앱 저장소에는 영향 없음(단, `assetlinks.json`은 되돌리면 placeholder로 복귀). 서명 키(`android.keystore`)를 지우면 이 패키지명으로 만든 서명 빌드를 다시는 만들 수 없음 — 지우기 전 반드시 백업 확인.
- User decision: 사용자가 "서명 이런건 니가하셈"으로 명시적으로 위임. 저장소 위치(별도 저장소)는 AskUserQuestion으로 확인받음. Play Console 앱 등록·인앱상품 생성·서비스 계정·내부테스트 업로드는 사용자 몫으로 README에 남김.

## 2026-09-11 — Claude: FINAL 인터랙티브 모의면접을 실제로 만들기 시작 (착수 기록, 작업 전)

- Agent/session: Claude, 같은 세션. 사용자가 "웹 파이날 결제 런칭부터하자 · 파이날 다된거제?"라고 물어서 조사했더니, 가격표(`pricing-comparison.tsx:55-59`)가 약속하는 "인터랙티브 AI 모의면접·답변 평가·동적 꼬리질문·취약질문 재훈련·최종 리포트" 5개가 코드에 전혀 없다는 걸 발견(정적 `interviewQuestions` 목록만 있고 PRO와 동일). 이 불일치는 **2026-08-24 change-log에 이미 기록돼 있었고**("모의면접은 턴 주고받기·답변 평가·재훈련이 필요한 사실상 별개 제품") 미뤄진 채 방치돼 있었음. 사용자에게 "가격표를 실제 기능에 맞게 낮추기 vs. 진짜로 만들기" 선택지를 물었고, 사용자가 "2번 md보고 구현해야겟네"로 실제 구현을 선택.
- Status: active, 착수. 같은 작업트리(`feature/google-play-billing` 브랜치, 커밋 안 함)에 Google Play Billing 작업과 함께 uncommitted 상태로 진행 — 서로 다른 파일이라 커밋 시점에 분리 가능(Google Play 쪽 파일 목록은 위 09-11 항목들 참고).
- Protected baseline: 기존 정적 면접질문 경로 전체 — `src/server/ai/quick/schema.ts`의 `interviewQuestionOutputSchema`/`interviewQuestions`/`interviewRisks`/`interviewerFlags`, `src/domain/result-document.ts`의 `interviewQuestionSchema`, `src/components/final-verification.tsx`의 정적 INTERVIEWER 패널, `result-workspace-complete.tsx`의 기존 "면접 준비" 탭(PRO도 공유) — 전부 무수정, 병행 추가만 한다. `openai-responses-gateway.ts`(QUICK/PRO/FINAL 분석 본체)도 무수정 — 새 기능은 `final-patch-gateway.ts`와 같은 독립 게이트웨이 파일로 만든다(이 코드베이스의 기존 관례, `resume-build-gateway.ts:12-19` 코멘트가 이유를 명시).
- Change and reason(계획): 상세 계획은 `C:\Users\jeonm\.claude\plans\crispy-yawning-avalanche.md`. 요약 — 신규 마이그레이션(`interview_sessions`/`interview_turns`/`interview_turn_attempts`, `analysis_runs`/`analysis_run_attempts`와 동일한 관례로 작성), 신규 AI 게이트웨이(`src/server/ai/interview/`, 동기 단일 호출 패턴, 백그라운드+폴링 방식 아님), 신규 API 라우트 3개(`/api/interview/start|turn|finish`), 신규 UI 컴포넌트(`interactive-interview.tsx`) + FINAL 전용 새 탭. 첫 질문은 이미 결제된 FINAL 분석 결과의 정적 `interviewQuestions[0]`을 재사용해 AI 호출 없이 시작(비용 절감). 턴 상한(세션당 5, 분석당 세션 3개)으로 비용 캡. 숫자 점수·합격확률 없음(AGENTS.md 원칙). "취약질문 재훈련"은 별도 시스템 없이 같은 세션 메커니즘을 약점 질문으로 재시작하는 식으로 해결.
- Files/branch: 아직 파일 변경 없음(이 항목이 착수 기록). 이후 커밋에서 구체적 파일 목록을 추가한다.
- Validation: 착수 시점, 해당 없음. 완료 시 `npx tsc --noEmit`, lint, vitest로 검증 예정 — 단, **실제 OpenAI 호출로 최소 1턴 검증은 코드 완성과 별개로 반드시 필요**(2026-09-06 FINAL 환불 사고가 "고쳤지만 실결제로 재검증 안 함"으로 재발 위험이 남았던 전례 참고).
- Rollback/recovery reference: 전부 신규 파일 추가(기존 분석 파이프라인·정적 면접질문 로직 무수정)라, 되돌릴 때는 나열된 신규 파일만 지우면 됨. 마이그레이션도 신규 파일이라 원격에 push 전이면 파일만 지우면 DB엔 흔적 없음.
- User decision: 사용자가 두 옵션(가격표 수정 vs. 실제 구현) 중 AskUserQuestion 거절 후 직접 "2번"으로 명시적 선택. UI 배치(새 탭 vs. 기존 탭 통합)도 "1번"(새 탭)으로 명시적 선택 — 모바일 UI 정리는 별개 과제이고 이번 결정은 FINAL을 약속대로 완성하려는 목적이라고 직접 확인.

## 2026-09-11 — Claude: FINAL 인터랙티브 모의면접 코드 완료 (DB 마이그레이션은 아직 원격에 push 안 함)

- Agent/session: Claude, 같은 세션, 바로 위 착수 기록의 계획(`crispy-yawning-avalanche.md`)을 그대로 구현.
- Status: active, `feature/google-play-billing` 브랜치 작업트리에 uncommitted 상태(Google Play Billing 작업과 파일이 겹치지 않아 커밋 시점에 분리 가능). **DB 마이그레이션은 `--dry-run`으로 정상 적용 확인만 했고 실제 push는 안 함** — 사용자 승인 후 진행.
- Protected baseline: 착수 기록과 동일. `git diff`로 정적 면접질문 경로(`quick/schema.ts`, `result-document.ts`의 `interviewQuestionSchema`, `final-verification.tsx`)와 `openai-responses-gateway.ts` 무수정 확인. `result-workspace-complete.tsx`는 새 탭 버튼·view 블록 두 줄만 추가, 기존 탭 로직은 한 글자도 안 건드림.
- Change and reason: 계획대로 구현.
  - **DB**: `supabase/migrations/20260911020000_interview_sessions.sql` — `interview_sessions`/`interview_turns`/`interview_turn_attempts`(analysis_run_attempts와 동일하게 service_role 전용 append-only 원장) 신규 테이블, `begin_interview_session`/`record_interview_turn`/`finish_interview_session` 3개 RPC(전부 `security definer set search_path=''`, 소유권·턴 예산을 `for update` 락으로 원자적 확인). 세션당 턴 상한(질문 수+2, 최대 5), 분석 실행당 세션 상한 3개.
  - **AI 게이트웨이**(신규, `final-patch-gateway.ts`와 동일한 동기 단일 호출 패턴 — 배경/폴링 아님): `src/server/ai/interview/interview-turn-gateway.ts`(답변 평가+다음 질문), `interview-report-gateway.ts`(세션 종료 후 1회, 턴 예산에 안 들어감). 둘 다 JSON Schema를 직접 손으로 써서 `strict:true`로 보냄(final-patch-gateway.ts와 동일 방식, `toOpenAIStrictSchema()` 안 씀). 숫자 점수·합격확률 필드 없음.
  - **비용 원장**: `src/server/analysis/interview-attempt-ledger.ts` — `attempt-ledger.ts`의 `recordAnalysisAttempt`를 그대로 복제한 모양(실패해도 턴 진행을 막지 않음, 토큰 없으면 null로 남김).
  - **API 라우트**(신규): `/api/interview/start`(AI 호출 없음 — 이미 결제된 FINAL 분석의 정적 `interviewQuestions[0]`을 첫 질문으로 씀), `/api/interview/turn`(컨텍스트를 클라이언트가 아니라 서버가 DB에서 다시 읽음 — final-patch 라우트와 같은 이유), `/api/interview/finish`.
  - **도메인 타입**: `src/domain/interview.ts` — 서버·클라이언트 공유 zod 스키마.
  - **UI**: `src/components/interactive-interview.tsx`(+`.module.css`, `final-wrap-up.module.css`와 색·톤 맞춤) — 시작 → 질문/답변 반복 → 최종 리포트(강점/약점/다음 행동, 숫자 없음) → "약점만 다시 연습"(같은 세션 메커니즘을 `focusQuestionIds`로 재사용, 새 서브시스템 안 만듦) 흐름. `result-workspace-complete.tsx`에 FINAL 전용 "모의면접" 탭 추가(기존 "면접 준비" 탭은 무수정).
- Files: 신규 — `supabase/migrations/20260911020000_interview_sessions.sql`, `src/domain/interview.ts`, `src/server/ai/interview/interview-turn-gateway.ts`(+`.test.ts`), `interview-report-gateway.ts`(+`.test.ts`), `src/server/analysis/interview-attempt-ledger.ts`(+`.test.ts`), `src/app/api/interview/{start,turn,finish}/route.ts`, `src/components/interactive-interview.tsx`(+`.module.css`). 수정 — `src/components/result-workspace-complete.tsx`(탭 추가).
- Validation: `npx tsc --noEmit -p .` clean, eslint 0건, `npx vitest run` 143 files·**1128 tests** 전부 통과(신규 11건, 기존 회귀 없음), `npm run build` 성공(`/api/interview/start|turn|finish` 라우트 정상 생성 확인). `supabase db push --dry-run` 정상(적용 가능 확인, 실제 push는 안 함).
...
## 2026-09-12 — Claude: 모의면접 무료 재시도를 버튼별 1회로 바꾸고, 3,000원 유료 재시도 결제 추가

- Agent/session: Claude, 같은 세션. 어제 스코프 아웃했던 유료 재시도를 사용자가 다시 요청 — "처음부터 다시"/"약점만 다시" 각각 1회 무료(현재는 합쳐서 3개 상한이라 한쪽만 계속 쓰면 다른 쪽은 무료로 못 씀), 그 이후엔 3,000원 결제. AskUserQuestion으로 두 가지 확인: (1) 버튼별 1회씩 보장 — "네", (2) 지금 바로 구현 — "네".
- Status: active. `supabase/migrations/20260912020000_interview_retry_entitlements.sql` 원격 반영 완료. 코드는 커밋 예정. **Polar `POLAR_INTERVIEW_RETRY_PRODUCT_ID`와 Google Play `GOOGLE_PLAY_INTERVIEW_RETRY_PRODUCT_ID`(+ 클라이언트용 `NEXT_PUBLIC_GOOGLE_PLAY_INTERVIEW_RETRY_PRODUCT_ID`) 둘 다 아직 실제 상품이 없어 실결제 테스트 불가 — 사용자가 두 대시보드에 3,000원 상품을 만들어야 함.**
- Protected baseline: `billing_orders`/`analysis_entitlements`(QUICK/PRO/FINAL 전용, `begin_quick_analysis`가 소비)와 `polar-webhook.ts`/`polar-checkout.ts`(글자수 견적 검증 경로) 전부 무수정 — 이 결제는 그 흐름과 완전히 독립된 새 표 2개(`interview_entitlements`, `interview_retry_orders`)로 처리한다. Polar 확인은 웹훅이 아니라 `polar-checkout-reconciliation.ts`가 이미 쓰는 "checkouts.get + orders.list로 직접 확인" 방식을 새 파일에 그대로 복제해서 쓴다(그 파일도 무수정, import 안 함 — 완전히 새 파일).
- Change and reason:
  - **무료 재시도 버튼별 분리**: `begin_interview_session`을 다시 작성 — 예전엔 분석 1건당 세션 3개 상한(합산)이었는데, 이제 `interview_entitlements`(analysis_run당 1행)에 `restart_free_used`/`weak_retry_free_used`를 각각 추적. 처음 세션은 항상 무료(FINAL에 이미 포함), 그다음부터 종류별(`p_focus_question_ids`가 null이면 "처음부터", 있으면 "약점만") 무료 1회 → 소진되면 `paid_extra_sessions`(있으면 차감) → 그것도 없으면 `PAYMENT_REQUIRED` 예외.
  - **결제 — Polar**: `src/server/billing/interview-retry-checkout.ts`(신규, 완전 독립) — 3,000원 고정가 체크아웃 생성(`createInterviewRetryCheckout`), 결제 후 돌아온 자리에서 Polar API로 직접 확인(`confirmInterviewRetryCheckout`, 웹훅 아님). 라우트 `/api/interview/retry/checkout`, `/api/interview/retry/confirm`(신규).
  - **결제 — Google Play**: `google-play-checkout.ts`에 `getInterviewRetryGooglePlayConfig`/`createAndroidPublisherClientForInterviewRetry` 추가(기존 QUICK/PRO/FINAL 사다리 함수는 무수정, 새 함수만 추가). `src/lib/google-play/purchase.ts`의 구매 로직을 `purchaseGooglePlayProduct` 공통 함수로 뽑아내고(기존 `purchaseProductViaGooglePlay` 동작은 그대로) `purchaseInterviewRetryViaGooglePlay` 신규 추가. 라우트 `/api/interview/retry/google-play/verify`(신규).
  - **지급 RPC**: `grant_interview_retry`(service_role 전용) — `(provider, provider_order_id)` 중복 방지 후 `interview_entitlements.paid_extra_sessions`를 +1. Polar/Google Play 양쪽 확인 경로가 이 함수 하나로 합류.
  - **UI**: `interactive-interview.tsx`에 `payment` phase 추가 — `PAYMENT_REQUIRED` 응답을 받으면 "3,000원 결제하고 다시하기" 버튼 노출, 클릭 시 Play 앱이면 바로 인앱결제, 웹이면 Polar 체크아웃으로 리다이렉트 후 `?interviewRetryCheckout=` 파라미터로 돌아오면 자동으로 결제 확인 → 원래 하려던 재시도(전체/약점만, sessionStorage에 기억)로 이어감.
- Files: 신규 — `supabase/migrations/20260912020000_interview_retry_entitlements.sql`, `src/server/billing/interview-retry-checkout.ts`, `src/app/api/interview/retry/{checkout,confirm,google-play/verify}/route.ts`. 수정 — `src/server/billing/google-play-checkout.ts`(함수 추가만), `src/lib/google-play/purchase.ts`(공통 함수 추출 + 함수 추가), `src/app/api/interview/start/route.ts`(PAYMENT_REQUIRED 코드 구분), `src/components/interactive-interview.tsx`(결제 phase).
...
## 2026-09-17 — Codex: user-approved release and one daily community post

- Agent: Codex. User explicitly approved Git commit/production deployment, then requested reducing automatic community articles to one per day and another commit.
- Status: popup committed as `9e81948`; daily limit implementation complete, validation/release in progress.
- Files: `src/app/api/community/seed/route.ts` and new route test; comment correction in `src/server/community/community-seed-content.ts`; new `supabase/migrations/20260917010000_community_seed_once_daily.sql`; this log.
- Behavior: stop before AI generation when at least one editorial post already exists in the existing UTC publication-day window. Keep one article and its existing editorial reply; retain 09:00 KST morning schedule and remove midday/evening schedules via migration. Existing articles, user comments and unrelated cron jobs are untouched. Atomic migration asserts exactly one active named seed schedule.
- Validation/release: boundary/auth/count-failure tests added; results pending. Supabase CLI currently reports missing platform access token; no migration has been applied yet. Cloudflare read-only deployment check succeeded; pre-release version is `2cbfa44f-51bd-4594-8d77-113d0cf129ae`. Release will be built from committed files only, excluding unrelated dirty legal changes. No manual paid AI generation planned.
- Recovery: revert only the daily-limit commit to restore the earlier application guard; restore the previous three cron schedules from migration `20260904030000` only if requested. Preserve `9e81948` as popup checkpoint. No existing migration was rewritten.

### 2026-09-17 — Production release verified; cron cleanup awaits Supabase login

- Status: Git commits `9e81948` (portrait homepage popup) and `684de5f` (one-post daily guard) deployed to the existing Cloudflare Worker `mooaresume`. Current production version: `59cb8fbc-8c82-42e5-a1c5-2bd8968f10bb`; Next build ID: `ghKzAICLv1gnXlwUYESg3`. Dashboard variables preserved using the existing `keep_vars: true` configuration. No GitHub push or merge performed.
- Build isolation: exported committed tree `684de5f`, excluding every unrelated dirty legal/checkpoint file. A dependency junction under `.wrangler` was rejected by Turbopack; materialized dependencies, then moved only this generated release copy outside the repository to `C:/Users/jeonm/AppData/Local/Temp/mooa-release-684de5f-20260917` with its own Git boundary to resolve a PostCSS scan timeout. Final OpenNext build passed with TypeScript and all 107 pages. Product source/config was not changed to work around the build environment. The temporary release remains outside test discovery; original workspace and creative artifacts are preserved.
- Checks: six new daily guard/auth/failure tests plus nine popup tests passed (15 total); focused ESLint passed. Earlier full suite: 146 files/1,146 tests before the six new tests. Production browser confirmed automatic muted playback of the 60.01-second container at 720x1280, close removes the video, and explicit replay reopens it. Public MP4 HEAD returned HTTP 200 and video/mp4. Cloudflare uploaded 19 changed assets and reported successful deployment, 50 ms Worker startup.
