...
/**
 * 결제 후 리포트 상단의 캐릭터 카드. 캐릭터 카드 체계가 있는 검사(직업흥미·직업가치)만
 * 만들고, 업무성향·종합은 항상 null이다. 이 탭에 검사 응답이 없어도 null(카드만 생략).
 */
export function computeReportHero(scope: string, interestRaw: string | null, valuesRaw: string | null): ReportHero | null {
  try {
