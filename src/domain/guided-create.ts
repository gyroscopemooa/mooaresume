...
/**
 * A block is one piece of material the applicant supplied. Experiences are
 * addressed by position — `experience-0` — because there is no fixed number of
 * them: an application with four questions needs four different experiences,
 * and two hardcoded slots forced the same story to be reused, which is the
 * duplication this product reports as a defect.
 */
export type GuidedBlockId = "motivation" | "aspiration" | "strength" | "goal" | `experience-${number}`;

export const EXPERIENCE_BLOCK_PREFIX = "experience-";
export const MAX_GUIDED_EXPERIENCES = 5;

export function experienceBlockId(index: number): GuidedBlockId {
  return `${EXPERIENCE_BLOCK_PREFIX}${index}`;
}

export function experienceBlockIndex(block: string): number | null {
  if (!block.startsWith(EXPERIENCE_BLOCK_PREFIX)) return null;
  const index = Number(block.slice(EXPERIENCE_BLOCK_PREFIX.length));
...
export const guidedCreateDraftSchema = z.object({
  ...
  experiences: z.array(guidedExperienceSchema).max(MAX_GUIDED_EXPERIENCES).default([]),
...
export function createGuidedExperience(): GuidedExperience {
  return guidedExperienceSchema.parse({});
}

export function createGuidedCreateDraft(): GuidedCreateDraft {
  return guidedCreateDraftSchema.parse({ experiences: [createGuidedExperience()] });
}
...
const FIXED_BLOCK_LABEL: Record<string, string> = {
  motivation: "지원 계기",
  aspiration: "하고 싶은 일",
  strength: "강점과 일하는 방식",
  goal: "입사 후 목표",
...
const EXPERIENCE_ORDINAL = ["①", "②", "③", "④", "⑤"];

export function guidedBlockLabel(block: string): string {
  const index = experienceBlockIndex(block);
  if (index !== null) return `경험 ${EXPERIENCE_ORDINAL[index] ?? index + 1}`;
  return FIXED_BLOCK_LABEL[block] ?? block;
}

export type GuidedField =
  | { kind: "text"; path: "motivation" | "aspiration" | "strength" | "goal"; label: string; placeholder: string; rows: number }
  | { kind: "experience"; index: number; field: keyof GuidedExperience; label: string; placeholder: string; rows: number };
...
export type GuidedStep = {
  ...
  optional?: boolean;
  fields: GuidedField[];
  /** Set on an experience step so the screen can offer the category chips. */
  experienceIndex?: number;
  /** Set on the last step of an experience so another one can be added there. */
  offersAnotherExperience?: boolean;
};

const ORDINAL = ["첫 번째", "두 번째", "세 번째", "네 번째", "다섯 번째"];

/**
 * The steps are built from the draft, not fixed, because the number of
 * experiences is up to the applicant. Every experience is asked the same four
 * questions — the earlier form interviewed the first one properly and squeezed
 * the second onto a single screen, which read as "the second one matters less"
 * when it is often the better story.
 */
export function buildGuidedSteps(draft: GuidedCreateDraft): GuidedStep[] {
  const count = Math.max(1, draft.experiences.length);
  const experienceSteps = Array.from({ length: count }).flatMap((_, index) => {
    const nth = ORDINAL[index] ?? `${index + 1}번째`;
    const optional = index > 0;
    return [
      {
        id: `experience-${index}-where`,
        title: `${nth} 경험은 언제, 어디서 한 일인가요?`,
        help: "종류를 먼저 고르고, 소속과 기간, 맡은 역할을 적어 주세요. 동아리·아르바이트·수업 과제도 좋은 소재가 됩니다.",
        optional,
        experienceIndex: index,
        fields: [{ kind: "experience" as const, index, field: "where" as const, label: "소속 · 기간 · 역할", placeholder: "예: 롯데테크 현장실습 · 2025.03~2025.08 · 안전관리 보조", rows: 2 }],
      },
      {
        id: `experience-${index}-situation`,
        // "무슨 문제가 있었나요"만 물으면 문제 해결형이 아닌 경험에서 없는
        // 문제를 지어내게 된다. 꾸준히 한 것, 처음 배운 것도 좋은 소재다.
        title: `${nth} 경험에서 무엇이 어려웠거나, 무엇을 새로 해야 했나요?`,
        help: "꼭 대단한 문제가 아니어도 됩니다. 처음 해보는 일이었거나, 손이 많이 갔거나, 사람을 설득해야 했던 것도 좋습니다.",
        optional,
        experienceIndex: index,
        fields: [{ kind: "experience" as const, index, field: "situation" as const, label: "어려웠던 점 · 새로 해야 했던 일", placeholder: "예: 같은 공정에서 같은 형태의 불량이 반복되는데 원인을 아무도 몰랐습니다.", rows: 4 }],
      },
      {
        id: `experience-${index}-action`,
        title: `${nth} 경험에서 본인이 실제로 한 행동은 무엇인가요?`,
        help: "팀이 한 일 말고 본인이 한 일을 적어 주세요. 작은 일도 괜찮습니다.",
        optional,
        experienceIndex: index,
        fields: [{ kind: "experience" as const, index, field: "action" as const, label: "내가 한 행동", placeholder: "예: 검사 기준서와 실제 작업 순서를 단계별로 직접 대조했습니다.", rows: 4 }],
      },
      {
        id: `experience-${index}-result`,
        title: "그래서 어떻게 됐고, 무엇을 배웠나요?",
        help: "숫자가 없으면 없는 대로 적어 주세요. 없는 수치는 만들지 않습니다.",
        optional,
        experienceIndex: index,
        offersAnotherExperience: index === count - 1 && count < MAX_GUIDED_EXPERIENCES,
        fields: [{ kind: "experience" as const, index, field: "result" as const, label: "결과와 배운 점", placeholder: "예: 점검 순서를 통일해 시험했고, 기준이 사람마다 다르면 문제가 반복된다는 걸 배웠습니다.", rows: 4 }],
      },
    ];
  });
...
export function guidedBlockText(draft: GuidedCreateDraft, block: string): string {
  const index = experienceBlockIndex(block);
  if (index !== null) {
    const experience = draft.experiences[index];
    return experience ? experienceText(experience) : "";
  }
  if (block === "motivation" || block === "aspiration" || block === "strength" || block === "goal") return draft[block].trim();
  return "";
}

export function hasGuidedBlock(draft: GuidedCreateDraft, block: string) {
  return guidedBlockText(draft, block).length > 0;
}

export function availableGuidedBlocks(draft: GuidedCreateDraft): GuidedBlockId[] {
  const blocks: GuidedBlockId[] = ["motivation", "aspiration"];
  draft.experiences.forEach((_, index) => blocks.push(experienceBlockId(index)));
  blocks.push("strength", "goal");
  return blocks.filter((block) => hasGuidedBlock(draft, block));
}
...
export function composeGuidedAnswer(draft: GuidedCreateDraft, blocks: readonly string[]): string {
  return blocks
    ...
    .map((block) => `[${guidedBlockLabel(block)}]\n${guidedBlockText(draft, block)}`)
    .join("\n\n");
}
