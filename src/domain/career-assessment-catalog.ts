...
export const careerAssessmentCatalog: readonly CareerAssessmentCatalogItem[] = [
  ...
  // 2026-09-18: 결과 화면(레이더 차트)이 갖춰져 있어 사용자 지시로
  // career-assessment-openness.ts와 함께 열었습니다("나머지 직업가치·업무성향도
  // 열어주고 즉시 런칭"). 캐릭터 카드는 아직 없어 직업흥미처럼 카드가
  // 붙지는 않습니다 — 점수·레이더차트만으로 먼저 공개합니다.
  ...
  { id:"onet-interest-ko", name:"O*NET 직업흥미 검사", formalName:"O*NET Interest Profiler · Korean adaptation", language:"ko", availability:"validation-required", method:"O*NET 번역·파생판", meta:"한국어 번역판 · 검증 전", purpose:"한국어 번역 문항으로 RIASEC 직업흥미를 탐색하는 정식판 후보입니다.", notice:"개발자 라이선스, 변경 고지, 대상 사용자 검증을 마친 뒤 공개합니다." },
  { id:"ipip-big-five-ko", name:"IPIP Big-Five 한국어판", formalName:"IPIP Big-Five Factor Markers · Korean version", language:"ko", availability:"validation-required", method:"IPIP 한국어 문항판", meta:"한국어 문항판 · 검토 중", purpose:"한국어 문항판과 점수 안내를 별도 버전으로 관리하는 경로입니다.", notice:"공개 문항 사용과 별개로, 한국어 결과의 해석 범위와 검증 기준을 먼저 정합니다." },
  { id:"onet-values-ko", name:"O*NET 직업가치 검사", formalName:"O*NET Work Importance Locator · Korean adaptation", language:"ko", availability:"validation-required", method:"O*NET 번역·파생판", meta:"한국어 번역판 · 검증 전", purpose:"한국어 번역 문항으로 직업가치 우선순위를 탐색하는 정식판 후보입니다.", notice:"개발자 라이선스, 변경 고지, 대상 사용자 검증을 마친 뒤 공개합니다." },
] as const;