...
export const createCommunityPostSchema = z.object({ topic: z.enum(communityTopics), title: z.string().trim().min(2, "제목을 두 글자 이상 입력해 주세요.").max(110), body: z.string().trim().min(5, "고민을 조금만 더 알려주세요.").max(5000), attachments: z.array(communityAttachmentSchema).max(3) });
...
export type CommunityPost = { id: string; topic: CommunityTopicId; title: string; body: string; anonymousAlias: string; recommendationCount: number; commentCount: number; createdAt: string; attachments: CommunityAttachment[] };
...
export const communityPreviewPosts: CommunityPost[] = [
  { id: "preview-role", topic: "career", title: "첫 지원 직무를 어떻게 정해야 할지 모르겠어요", body: "관심 있는 분야는 여러 개인데 지금 가진 경험으로 어디부터 지원해야 할지 막막해요.", anonymousAlias: "익명 준비자", recommendationCount: 12, commentCount: 3, createdAt: "방금 전", attachments: [] },
