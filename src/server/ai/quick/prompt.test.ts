...
describe("줄여도 되는 것과 안 되는 것", () => {
  ...
  it("채우기 모드의 지시는 그대로 유지된다", () => {
    const build = buildQuickAnalysisInstructions({
      ...
      product: "PRO",
      writingMode: "BUILD",
      questions: [{ id: "q1", title: "", prompt: "지원 동기를 서술하세요.", answer: "짧은 답변입니다.", targetLength: 700 }],
    });
...
describe("두괄식과 첨삭 요약", () => {
  ...
  it("지원서 전체에서 두괄식을 쓰는 문항 수를 제한한다", () => {
    // 문항별로만 판단하면 상한이 없어, 각자 "이 문항은 해당된다"고 보고
    // 전부에 걸릴 수 있다. 전부가 결론을 앞세우면 그게 다시 하나의 틀이다.
    const text = instructions();

    expect(text).toContain("한 문항, 많아야 두 문항에만 적용하세요");
    expect(text).toContain("장점·단점 문항이 있으면 그 문항을 먼저 선택");
  });
