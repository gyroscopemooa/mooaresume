...
${LENGTH_INTEGRITY_RULE}`,
    ...
    // heading.
    // 근거 없이 분량을 채우는 것은 이미 금지돼 있습니다. 그래서 짧게 돌아오는
    // 것은 결함이 아니라 의도인데, 그 사실을 말해 주지 않으면 손님에게는 그냥
    // "덜 해 준 것"으로 읽힙니다. 무엇을 알려주면 채울 수 있는지 이 칸에
    // 적으면, 손님이 그 정보를 주고 다시 받을 수 있습니다.
    "lengthNote: 첨삭본이 목표 글자 수보다 눈에 띄게 짧다면, 무엇을 알려주면 더 채울 수 있는지 한두 문장으로 적으세요. 근거 없이 채우지 않은 이유를 밝히고, 지원자가 실제로 줄 수 있는 정보(수치·기간·역할·결과 같은 것)를 구체적으로 물으세요. 목표를 충분히 채웠거나 더 채울 이유가 없으면 null을 반환하세요. 여기에 일반적인 조언('경험을 더 쓰세요')을 넣지 마세요 — 이 문항에서만 나올 수 있는 것을 물어야 합니다.",
    ...
    ...(revisionRequestText(request)
      ? [
          // 예전에는 "이전 첨삭 결과를 보고 다시"라고 못 박았습니다. 이제
          // 첫 분석에서도 요청을 받으므로(간편 입력의 "이렇게 써주세요"),
          // 있지도 않은 이전 결과를 모델이 전제하지 않도록 중립으로 둡니다.
          `지원자가 이번 첨삭에 요청사항을 남겼습니다. 요청사항: "${revisionRequestText(request)}"`,
