...
export function buildQuickAnalysisInstructions(request: AnalysisRequest) {
  return [
    ...
    "delete를 소극적으로 쓰지 마세요. 과장, 근거 없는 강점 주장, 앞뒤 논리의 불일치, 질문과 무관한 내용, 같은 경험·표현의 불필요한 반복, 불필요하게 공격적이거나 장황한 표현처럼 대부분의 평가자에게 위험하게 읽힐 문제가 있으면 분명하게 delete로 짚으세요. 소재 자체는 유효하고 표현이나 초점만 아쉬우면 delete가 아니라 revise입니다.",
    "교수·상사·동료 등 제3자의 평가나 피드백을 인용하는 것 자체는 delete 사유가 아닙니다. 자기 주장에 객관성을 더하거나 성장 계기를 보여준다면 긍정적으로 평가하세요. 그 피드백을 계기로 실제 행동이나 변화가 뒤따르지 않고 인용만 있을 때, 또는 같은 인용이 반복될 때만 revise나 vague를 고려하세요.",
