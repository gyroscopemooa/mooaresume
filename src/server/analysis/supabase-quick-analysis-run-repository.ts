...
export class SupabaseQuickAnalysisRunRepository implements QuickAnalysisRunRepository {
  ...
  async getRunningContext(analysisRunId: string) {
    ...
    const documentKind = (id: string) => { const value = kinds.get(id); if (value === "COVER_LETTER") return "cover_letter"; if (value === "JOB_POSTING") return "job_posting"; if (value === "RESUME") return "resume"; if (value === "CAREER_DOCUMENT") return "career_description"; if (value === "REVISION_REQUEST") return "revision_request"; if (value === "CERTIFICATE") return "certificate"; if (value === "APPLICANT_NOTE") return "applicant_note"; if (value === "OTHER") return "other"; return "portfolio"; };
    ...
    const supportingKinds = new Set(["resume", "career_description", "portfolio", "revision_request", "certificate", "applicant_note", "other"]);
