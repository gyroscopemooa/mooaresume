...
export function PortfolioBuildPanel({ variant = "default" }: { variant?: "default" | "app" } = {}) {
  ...
  function onBuyClick() {
    ...
    if (!signedIn) return;
    if (!enough) {
      setMessage("적어 주신 내용이 조금 더 필요합니다. 프로젝트 설명을 조금 더 적어 주세요.");
      return;
    }
    void startCheckout();
  }
      ...
      <div className={styles.card}>
        ...
        {signedIn === false && loginOpen && !inApp && <div ref={loginRef} className={`${styles.login} ${loginNudge ? styles.loginNudge : ""}`}>
          ...
          <div className={styles.loginSteps}>
            ...
            <span><i>2</i>결제 {PORTFOLIO_BUILD_PRICE_KRW.toLocaleString()}원</span>
          </div>
        ...
        <footer className={styles.footer}>
          ...
          <div className={styles.actions}>
            ...
            {inApp ? <AppPaidToolNotice tool="AI 포트폴리오 설명글" /> : <button
              type="button"
