...
import { ResultWorkspaceV2 } from "./result-workspace-v2";
import { sampleResultDocument } from "@/fixtures/result-document";

afterEach(cleanup);

describe("ResultWorkspaceV2 제출본 탭", () => {
  it("원문 위 마킹과 피드백 카드를 문항별로 보여준다", () => {
    render(<ResultWorkspaceV2 result={sampleResultDocument} />);
  ...
  it("originalAnnotations가 없는 예전 데이터도 화면이 깨지지 않는다", () => {
    ...
    render(<ResultWorkspaceV2 result={legacyResult} />);
    fireEvent.click(screen.getByRole("button", { name: "제출본" }));
