...
import { WORK_VALUE_BASE_CARDS, WORK_VALUE_ITEMS, type WorkValueAnswer } from "@/domain/career-work-values";
...
// 직업흥미의 RIASEC 미리보기와 같은 자리 — 시작 전 슬라이드용 단일 문자 6장.
const characterPreviews = Object.values(WORK_VALUE_BASE_CARDS);
...
export function CareerValuesReflection() {
  ...
  if (!started) return <main className={styles.intro}>
    <span className={styles.introBadge}>MOOA WORK VALUES · BETA</span><h1>일에서 무엇을<br /><em>포기하기 어려운가요?</em></h1><p>자율성·성장·안정·여유·의미·보상, 일에서 중요하게 보는 여섯 기준을 무아 자체 문항으로 살펴봅니다. 어떤 조건을 우선하는가를 탐색하는 베타 도구이며 표준화 검사나 채용 판정이 아닙니다.</p><div className={styles.introMeta}><span><Clock3 />약 4분</span><span>18문항</span></div>
    {hasCompletedResult ? <div className={styles.actionStack}><Link href="/career/values/result">이전 결과 다시 보기 <ArrowRight /></Link><button className={styles.startButton} type="button" onClick={() => { window.sessionStorage.removeItem(storageKey); setStarted(true); }}>새로 탐색하기 <ArrowRight /></button></div> : <button className={styles.startButton} type="button" onClick={() => setStarted(true)}>탐색 시작하기 <ArrowRight /></button>}
    <div className={styles.introNotes}><span><ShieldCheck />응답은 현재 브라우저에만 임시 저장</span></div>
    <section className={styles.method}><b>무엇을 알 수 있나요?</b><p>자율성·성장·안정·여유·의미·보상 여섯 기준 중 어떤 것을 상대적으로 더 중요하게 보는지 확인하고, 지원할 조직·역할에서 확인할 단서를 얻습니다.</p></section>
    <section className={styles.characterShowcase} aria-label="직업가치 캐릭터 미리보기"><div className={styles.characterShowcaseHead}><span>WORK VALUE CHARACTER PREVIEW</span><b>어떤 캐릭터가 나올지 미리 살펴보세요.</b><small>예시 캐릭터이며, 실제 결과는 응답한 상위 두 기준 조합으로 결정됩니다.</small></div><div className={styles.characterViewport}><div className={styles.characterTrack}>{[...characterPreviews, ...characterPreviews].map((character, previewIndex) => <article className={styles.characterCard} key={`${character.code}-${previewIndex}`} aria-hidden={previewIndex >= characterPreviews.length}><Image src={character.imagePath} alt={`${character.title} 직업가치 캐릭터 예시`} width={300} height={393} sizes="(max-width: 760px) 190px, 300px" quality={100} unoptimized /></article>)}</div></div></section>
  </main>;
