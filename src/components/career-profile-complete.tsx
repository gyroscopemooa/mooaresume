...
export function CareerProfileComplete() {
  ...
  return <main className={styles.result}>
    ...
    {completed > 0 && <section className={styles.useCard}><div><span className={styles.sectionKicker}>NEXT STEP</span><h2>결과를 지원서의<br />실제 경험과 연결하세요.</h2><p>검사 결과는 답을 대신하지 않습니다. 지금까지 했던 경험 중 어떤 환경에서 동기가 생겼는지, 어떤 문제를 해결하고 싶었는지 확인하는 질문으로 사용하세요.</p><ul><li><b>업무환경</b><span>내가 몰입하기 쉬운 협업 방식·자율성·구조는 무엇인가요?</span></li><li><b>직무 활동</b><span>흥미가 높은 활동을 실제로 해 본 경험과 배운 점은 무엇인가요?</span></li><li><b>지원 기준</b><span>지원할 공고에서 반드시 확인할 근무 조건과 역할은 무엇인가요?</span></li></ul></div><div className={styles.actionStack}><Link href="/onboarding"><FileText />이 결과로 자소서 분석하기 <ArrowRight /></Link><Link href="/entry"><LockKeyhole />로그인하고 결과 저장 준비 <ArrowRight /></Link></div></section>}
