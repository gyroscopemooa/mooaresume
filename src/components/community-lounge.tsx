...
function PostComposer({ onClose, onCreated, onError }: { onClose: () => void; onCreated: (post: CommunityPost) => void; onError: (message: string) => void }) {
  ...
  async function submit() { const normalizedTitle = title.trim(); const normalizedBody = body.trim(); if (normalizedTitle.length < 2) { onError("제목을 2자 이상 입력해 주세요."); return; } if (normalizedBody.length < 5) { onError("고민 내용을 5자 이상 입력해 주세요."); return; } setSaving(true); try { const attachments: CommunityAttachmentInput[] = []; for (const file of files) { const form = new FormData(); form.append("file", file); const response = await fetch("/api/community/uploads", { method: "POST", body: form }); const data = await responseJson(response) as ApiError & CommunityAttachmentInput; if (!response.ok) throw new Error(data.error ?? "첨부파일을 올리지 못했어요."); attachments.push(data); } const response = await fetch("/api/community/posts", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ topic, title: normalizedTitle, body: normalizedBody, attachments }) }); const data = await responseJson(response) as ApiError & { post?: CommunityPost }; if (!response.ok || !data.post) throw new Error(data.error ?? "글을 저장하지 못했어요."); onCreated(data.post); } catch (error) { onError(error instanceof Error ? error.message : "글을 저장하지 못했어요."); } finally { setSaving(false); } }
