      ...
      {view === "overview" && <section className={styles.overview}>
        ...
        <div className={styles.overviewGrid}>
          ...
          <aside>
            ...
            {result.referenceMaterials.length > 0 && <section className={styles.panel}><span className={styles.eyebrow}>참고자료 활용</span><p className={styles.referenceIntro}>첨부 자료에서 현재 문항과 연결되는 확인 가능한 사실을 찾아 검토했습니다.</p><details className={styles.referenceDetails}><summary><span>자료 활용 내역 보기</span><small>파일별 확인</small></summary><div>{result.referenceMaterials.map((material) => <div className={styles.referenceMaterial} key={material.id} data-status={material.status}><FileText/><span><b>{material.filename}</b><small>{material.label} · {material.note}</small></span><em>{material.status === "read" ? "읽음" : material.status === "partial" ? "일부 읽음" : "미포함"}</em></div>)}</div></details></section>}
        ...
        {result.questions.map((question) => {
          ...
          return <article className={styles.question} key={question.id}>
            ...
            {showLengthGap && <div className={styles.lengthGap}>
              ...
              <p>{question.lengthNote ?? "현재 결과에 추가할 확인 가능한 사실이 충분하지 않았습니다. 근거 없이 채우면 오히려 감점이라 늘리지 않았어요."}</p>
