// Verbatim port of ResultWorkspaceV2 as it existed on the 2026-08-20 `feature/original-annotations`
// branch (commit 26bb5df / 68807e1) — full multi-tab layout including the "제출본" tab, kept as its
// own component so it can be reached from a comparison button without touching the active
// `result-workspace-v2.tsx` or its shared domain/fixture files. See docs/agent-change-log.md.
"use client";
...
import { buildFinalDocumentText, countCompactCharacters, type ResultDocument, type ResultOriginalAnnotation } from "@/domain/result-document-claude-restored";
import { claudeRestoredSampleDocument } from "@/fixtures/result-document-claude-restored";
...
import styles from "./result-workspace-claude-restored.module.css";
...
export function ResultWorkspaceClaudeRestored({ result = claudeRestoredSampleDocument }: { result?: ResultDocument }) {
  const storageKey = "mooa:result-edits:claude-restored:" + result.caseId + ":v1";
  ...
  return <main className={styles.page}>
    <header className={styles.header}>
      ...
      <div><em className={styles.badge}>CLAUDE 복원판 · 비교용{result.isSample ? " · 샘플" : ""}</em><button onClick={() => copy("all", finalText)}>{copied === "all" ? <Check/> : <Clipboard/>}{copied === "all" ? "복사됨" : "전체 복사"}</button><button onClick={download}><Download/> TXT 저장</button></div>
    </header>

    <div className={styles.container}>
      <Link href="/result" className={styles.back}><ArrowLeft/> 현재 결과 화면으로</Link>
