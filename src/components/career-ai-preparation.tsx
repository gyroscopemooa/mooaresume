...
import { ArrowRight, BrainCircuit, CheckCircle2, FileText, LayoutDashboard, LockKeyhole, ShieldCheck, Target } from "lucide-react";
...
export function CareerAiPreparation({ scope }: { scope: Scope }) {
  ...
  const complete = useMemo(() => assessments.filter((_, index) => Boolean([interestRaw, workStyleRaw, valuesRaw][index])), [interestRaw, workStyleRaw, valuesRaw]);
  ...
  if (signedIn === false) return <AiFrame eyebrow="AI CAREER INSIGHTS" title="결과를 안전하게 저장하고\n해설 범위를 골라 주세요." description="로그인하면 현재 기기에서 완료한 검사 결과를 확인하고, 종합 또는 개별 해설을 선택할 수 있어요." status="로그인 필요"><Link className={styles.primary} href={`/career/login?next=/career/ai?scope=${scope}`}>로그인하고 심층해설 확인하기 <ArrowRight /></Link><Link className={styles.ghostLink} href="/career">커리어 검사 홈으로</Link></AiFrame>;
  ...
  return <AiFrame eyebrow="AI CAREER INSIGHTS" title={title} description={description} status="출시 준비 중">
    <section className={styles.analysisGrid}>
      ...
      <aside className={styles.packageCard}>
        ...
        {scope === "interest" && <Link className={styles.sampleButton} href="/career/ai/sample?scope=interest">심층해설 예시 보기 <ArrowRight /></Link>}
