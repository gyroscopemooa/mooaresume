...
export function CareerInterestAssessment() {
  ...
  if (!started) return <main className={styles.intro}>
    <span className={styles.introBadge}>MOOA CAREER INTEREST · BETA</span><h1>어떤 활동에<br /><em>에너지가 생기나요?</em></h1><p>RIASEC(Holland) 직업흥미 모델의 여섯 활동 영역을 참고해 만든 무아 자체 문항입니다. 이 검사는 무엇을 해 보고 싶은가를 살펴봅니다. 좋아하는 활동의 방향을 탐색하는 베타 도구이며, 표준화 검사·직업 추천·채용 판정이 아닙니다.</p><div className={styles.introMeta}><span><Clock3 />약 5분</span><span>30문항</span><span>무료 베타</span></div><button type="button" className={styles.startButton} onClick={() => setStarted(true)}>탐색 시작하기 <ArrowRight /></button><div className={styles.introNotes}><span><ShieldCheck />응답은 현재 브라우저에만 임시 저장</span></div><section className={styles.method}><b>무엇을 알 수 있나요?</b><p>현실형(R)·탐구형(I)·예술형(A)·사회형(S)·진취형(E)·관습형(C) 활동 중 어떤 활동에 상대적으로 흥미가 높은지 확인하고, 직무·경험을 탐색할 단서를 얻습니다.</p></section>
    <section className={styles.characterShowcase} aria-label="RIASEC 캐릭터 미리보기"><div className={styles.characterShowcaseHead}><span>RIASEC CHARACTER PREVIEW</span><b>어떤 캐릭터가 나올지 미리 살펴보세요.</b><small>예시 캐릭터이며, 실제 결과는 응답한 상위 활동 조합으로 결정됩니다.</small></div><div className={styles.characterViewport}><div className={styles.characterTrack}>{[...characterPreviews, ...characterPreviews].map((character, index) => <article className={styles.characterCard} key={`${character.code}-${index}`} aria-hidden={index >= characterPreviews.length}><Image src={character.image} alt={`${character.label} RIASEC 캐릭터 예시`} width={300} height={393} sizes="(max-width: 760px) 190px, 300px" quality={100} unoptimized /></article>)}</div></div></section>
  </main>;
