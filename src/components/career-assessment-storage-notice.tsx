...
export function CareerAssessmentStorageNotice({ assessmentCode, assessmentVersion, answersRaw, resultPath, restored = false }: { assessmentCode: AssessmentCode; assessmentVersion: string; answersRaw: string | null; resultPath: string; restored?: boolean }) {
  ...
  return <section className={styles.notice}><Cloud /><span><b>{state === "unavailable" ? "계정 저장 데이터베이스가 아직 연결되지 않았어요." : "결과를 계정에 저장하지 못했어요."}</b><small>{state === "unavailable" ? "지금은 이 브라우저에만 보관됩니다. 데이터베이스 연결 후 다음 결과부터 계정에 저장할 수 있어요." : "현재 결과는 이 브라우저에 그대로 남아 있습니다. 로그인 상태와 연결을 확인한 뒤 결과 페이지를 다시 열어 주세요."}</small></span></section>;
}
