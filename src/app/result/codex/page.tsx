...
import { ResultWorkspaceCodex } from "@/components/result-workspace-codex";
import { resultDocumentSchema } from "@/domain/result-document";
import { sampleResultDocument } from "@/fixtures/result-document";
import { createClient } from "@/lib/supabase/server";

export default async function CodexResultPage({
  searchParams,
}: {
  searchParams: Promise<{ analysisRunId?: string }>;
}) {
  const { analysisRunId } = await searchParams;
  if (!analysisRunId) return <><ResultVariantNav active="codex"/><ResultWorkspaceCodex result={sampleResultDocument}/></>;
  ...
  return <><ResultVariantNav active="codex" analysisRunId={analysisRunId}/><ResultWorkspaceCodex result={parsed.data}/></>;
}
