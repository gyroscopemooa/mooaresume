...
export async function GET(_request: NextRequest, context: { params: Promise<{ postId: string }> }) {
  ...
  const { data, error } = await supabase.from("community_comments").select("id, body, anonymous_alias, created_at").eq("post_id", await postId(context)).eq("status", "PUBLISHED").order("created_at", { ascending: true });
...
export async function POST(request: NextRequest, context: { params: Promise<{ postId: string }> }) {
  ...
  const { data, error } = await supabase.from("community_comments").insert({ post_id: await postId(context), body: parsed.data.body }).select("id, body, anonymous_alias, created_at").single();
