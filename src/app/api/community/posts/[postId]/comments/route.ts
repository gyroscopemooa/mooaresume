...
export async function GET(_request: NextRequest, context: { params: Promise<{ postId: string }> }) {
  ...
  const { data, error } = await supabase.from("community_comments").select("id, owner_user_id, body, anonymous_alias, is_editorial, created_at").eq("post_id", await postId(context)).eq("status", "PUBLISHED").order("created_at", { ascending: true });
  if (error) return NextResponse.json({ error: "댓글을 불러오지 못했습니다." }, { status: 500 });
  return NextResponse.json({ comments: (data ?? []).map((row) => toCommunityComment(row as Record<string, unknown>, viewerUserId)) });
}
export async function POST(request: NextRequest, context: { params: Promise<{ postId: string }> }) {
  ...
  const { data, error } = await supabase.from("community_comments").insert({ post_id: await postId(context), body: parsed.data.body }).select("id, owner_user_id, body, anonymous_alias, is_editorial, created_at").single();
  if (error || !data) return NextResponse.json({ error: "댓글을 저장하지 못했습니다." }, { status: 500 });
  return NextResponse.json({ comment: toCommunityComment(data as Record<string, unknown>, auth.user.id) }, { status: 201 });
}