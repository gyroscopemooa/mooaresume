...
import { legalDocumentTypeSchema, LEGAL_DOCUMENT_BUILD_PRICE_KRW } from "@/domain/legal-case";
...
export async function POST(request: NextRequest, context: { params: Promise<{ caseId: string }> }) {
  ...
  try {
    const build = await createBuild(legalDocumentBuild.table, guard.userId, LEGAL_DOCUMENT_BUILD_PRICE_KRW, {
      case_id: caseId,
    ...
    const session = await createDocumentBuildCheckout(legalDocumentBuild.product, {
      buildId: build.id,
      userId: guard.userId,
      priceKrw: LEGAL_DOCUMENT_BUILD_PRICE_KRW,
    ...
    return NextResponse.json({ buildId: build.id, checkoutUrl: session.checkoutUrl, priceKrw: LEGAL_DOCUMENT_BUILD_PRICE_KRW }, { status: 201 });
  } catch (error) {
