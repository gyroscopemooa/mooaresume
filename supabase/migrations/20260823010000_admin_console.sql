...
--
-- Every statement is idempotent. mail_send_log was created by hand on the
-- remote database before this file was ever pushed, so a plain create aborted
-- the whole push and blocked thirteen later migrations behind it. Nothing
-- about the schema changed here — only the ability to run this file twice.
...
create index if not exists mail_send_log_sent_idx on public.mail_send_log(sent_at desc);
create index if not exists mail_send_log_batch_idx on public.mail_send_log(batch_id, recipient);
create index if not exists mail_send_log_recipient_idx on public.mail_send_log(recipient, sent_at desc);

-- No `if not exists` for types, so the duplicate is caught instead.
do $$ begin
