-- Lets an applicant re-run an analysis with an instruction attached.
--
-- After reading a finished result, the thing someone wants to say is usually
-- not another fact about themselves but a direction for the draft: "에이텍
-- 내용은 빼고 관련 직무로만 구성해주세요". That is a different kind of input
-- from a résumé, and it has to stay different all the way down — a material
-- answers "what else is true about me", an instruction answers "what should
-- this draft do differently". Filed as a material, "leave X out" reads to the
-- model as more material about X.
--
-- It also must never become quotable evidence. See NON_EVIDENCE_KINDS in
-- src/server/ai/quick/validator.ts, which excludes it alongside the posting.

-- Enum values cannot be added inside a transaction that then uses them, so
-- this runs on its own before the function below is redefined.
alter type public.document_kind add value if not exists 'REVISION_REQUEST';
...
/*
 * Same body as 20260821030000, with two changes: REVISION_REQUEST maps to its
 * own request kind, and it is gated to PRO like the other extras.
 *
 * The `else 'portfolio'` fallback is why this cannot be left alone — an
 * unmapped REVISION_REQUEST would arrive as a portfolio, which is both
 * supporting material and quotable evidence. That is the exact opposite of
 * what it is.
 */
...
begin
  ...
  where si.snapshot_id = target_run.submission_snapshot_id
    ...
    and (d.kind not in ('OTHER', 'REVISION_REQUEST') or target_run.product = 'PRO');
