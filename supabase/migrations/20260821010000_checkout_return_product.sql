...
-- The checkout-return screen only had client-side (sessionStorage) knowledge
-- of whether a run was QUICK or PRO, which is unreliable after a redirect to
-- Polar and back. Expose the server-confirmed product so the return screen
-- can show what was actually purchased instead of guessing.
create or replace function public.get_quick_checkout_return(p_provider_checkout_id text)
returns jsonb
language plpgsql
security invoker
set search_path = ''
...
begin
  ...
  select ar.status, ar.product into run_status, run_product
