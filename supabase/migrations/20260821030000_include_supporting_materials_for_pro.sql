...
-- PRO collects 이력서·경력기술서·포트폴리오·경험·자유 메모 and stores them as
-- documents of kind 'OTHER', but this function excluded that kind outright.
-- Every PRO promise that depends on those materials — 자료 간 충돌 검사,
-- 근거 보완, 더 적합한 경험 추천, 공고 ↔ 경험 매칭 — was therefore impossible
-- to deliver: the model never received the material at all.
--
-- Supporting documents are now included for PRO runs only, so the QUICK scope
-- ("작성한 글만 첨삭") stays exactly as advertised.
...
begin
  ...
  where si.snapshot_id = target_run.submission_snapshot_id
    ...
    and (d.kind <> 'OTHER' or target_run.product = 'PRO');
