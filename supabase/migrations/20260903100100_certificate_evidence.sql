 ...
 * 자격·증명서를 제 이름으로 부릅니다.
 *
 * `CERTIFICATE`가 생기기 전, 자격증·면허증·졸업증명서는 전부 `OTHER`였습니다.
 * `OTHER`는 PRO·FINAL에서 제외되지는 않았지만 두 가지가 어긋나 있었습니다:
 *
 *  - 모델에게 **'portfolio'** 라는 이름으로 전달됐습니다. 증빙 서류가 작품집으로
 *    소개되면, 그것을 근거로 쓰라는 신호가 사라집니다.
 *  - 예산 순서가 **맨 뒤**였습니다. 글자 예산이 넘치면 제일 먼저 잘리는데,
 *    자료를 많이 넣는 사람일수록 잘립니다. 그 사람이 FINAL 손님입니다.
 *
 * 바꾸는 것은 이 둘뿐입니다. 기존 갈래의 순서와 이름, 예산 계산식, 이용권 처리,
 * 무엇이 보이고 무엇이 잘리는지의 규칙은 그대로입니다.
 */
  ...
  with visible as (
    select
      ...
      case d.kind
        ...
        when 'CAREER_DOCUMENT' then 4
        when 'REVISION_REQUEST' then 5
        when 'PORTFOLIO' then 6
